...
static int32_t ms_zrtp_srtpSecretsAvailable(void* clientData, const bzrtpSrtpSecrets_t *secrets, uint8_t part) {
	...
	ms_message("ZRTP secrets are ready for %s; auth tag algo is %s and cipher algo is %s", (part==ZRTP_SRTP_SECRETS_FOR_SENDER)?"sender":"receiver", bzrtp_algoToString(secrets->authTagAlgo), bzrtp_algoToString(secrets->cipherAlgo));


	if (part==ZRTP_SRTP_SECRETS_FOR_RECEIVER) {
		uint8_t *key = (uint8_t *)ms_malloc0((secrets->peerSrtpKeyLength+secrets->peerSrtpSaltLength)*sizeof(uint8_t));
		...
		if (secrets->authTagAlgo == ZRTP_AUTHTAG_HS32){
			...
			if (secrets->cipherAlgo == ZRTP_CIPHER_AES3){
				ms_media_stream_sessions_set_srtp_recv_key(userData->stream_sessions, MS_AEAD_AES_256_GCM, (const char *)key, (secrets->peerSrtpKeyLength+secrets->peerSrtpSaltLength), MSSRTP_ALL_STREAMS);
			}else{
				ms_media_stream_sessions_set_srtp_recv_key(userData->stream_sessions, MS_AEAD_AES_128_GCM, (const char *)key, (secrets->peerSrtpKeyLength+secrets->peerSrtpSaltLength), MSSRTP_ALL_STREAMS);
			}
		} else {
	...
	if (part==ZRTP_SRTP_SECRETS_FOR_SENDER) {
		...
		if (secrets->authTagAlgo == ZRTP_AUTHTAG_HS32){
			...
			if (secrets->cipherAlgo == ZRTP_CIPHER_AES3){
				ms_media_stream_sessions_set_srtp_send_key(userData->stream_sessions, MS_AEAD_AES_256_GCM, (const char *)key, (secrets->selfSrtpKeyLength+secrets->selfSrtpSaltLength), MSSRTP_ALL_STREAMS);
			}else{
				ms_media_stream_sessions_set_srtp_send_key(userData->stream_sessions, MS_AEAD_AES_128_GCM, (const char *)key, (secrets->selfSrtpKeyLength+secrets->selfSrtpSaltLength), MSSRTP_ALL_STREAMS);
			}
		}else{
			ms_fatal("unsupported auth tag");
		}
...
static int ms_zrtp_getAlgoId(uint8_t algo) {
	switch(algo) {
		...
		case(ZRTP_AUTHTAG_GCM): return MS_ZRTP_AUTHTAG_GCM;
		...
		case(ZRTP_KEYAGREEMENT_HQC1): return MS_ZRTP_KEY_AGREEMENT_HQC1;
		case(ZRTP_KEYAGREEMENT_HQC2): return MS_ZRTP_KEY_AGREEMENT_HQC2;
		case(ZRTP_KEYAGREEMENT_HQC3): return MS_ZRTP_KEY_AGREEMENT_HQC3;
		case(ZRTP_KEYAGREEMENT_K255_KYB512): return MS_ZRTP_KEY_AGREEMENT_K255_KYB512;
		case(ZRTP_KEYAGREEMENT_K255_HQC128): return MS_ZRTP_KEY_AGREEMENT_K255_HQC128;
		case(ZRTP_KEYAGREEMENT_K448_KYB1024): return MS_ZRTP_KEY_AGREEMENT_K448_KYB1024;
		case(ZRTP_KEYAGREEMENT_K448_HQC256): return MS_ZRTP_KEY_AGREEMENT_K448_HQC256;
		case(ZRTP_KEYAGREEMENT_K255_KYB512_HQC128): return MS_ZRTP_KEY_AGREEMENT_K255_KYB512_HQC128;
		case(ZRTP_KEYAGREEMENT_K448_KYB1024_HQC256): return MS_ZRTP_KEY_AGREEMENT_K448_KYB1024_HQC256;
...
static void set_auth_tag_suites(bzrtpContext_t *ctx, const MSZrtpAuthTag *authTags, const MsZrtpCryptoTypesCount count) {
	...
	for (i=0; i < count; i++) {
		switch (authTags[i]) {
			...
			case MS_ZRTP_AUTHTAG_GCM:     bzrtpAuthTags[bzrtpCount++] = ZRTP_AUTHTAG_GCM; break;
		}
	}
...
static void set_key_agreement_suites(bzrtpContext_t *ctx, const MSZrtpKeyAgreement *keyAgreements, const MsZrtpCryptoTypesCount count) {
	...
	for (i=0; i < count; i++) {
		switch (keyAgreements[i]) {
			...
			case MS_ZRTP_KEY_AGREEMENT_K255:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K255; break;
			case MS_ZRTP_KEY_AGREEMENT_K448:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K448; break;
			case MS_ZRTP_KEY_AGREEMENT_KYB1:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_KYB1; break;
			case MS_ZRTP_KEY_AGREEMENT_KYB2:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_KYB2; break;
			case MS_ZRTP_KEY_AGREEMENT_KYB3:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_KYB3; break;
			case MS_ZRTP_KEY_AGREEMENT_HQC1:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_HQC1; break;
			case MS_ZRTP_KEY_AGREEMENT_HQC2:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_HQC2; break;
			case MS_ZRTP_KEY_AGREEMENT_HQC3:			bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_HQC3; break;
			case MS_ZRTP_KEY_AGREEMENT_K255_KYB512:     bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K255_KYB512; break;
			case MS_ZRTP_KEY_AGREEMENT_K255_HQC128:     bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K255_HQC128; break;
			case MS_ZRTP_KEY_AGREEMENT_K448_KYB1024:    bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K448_KYB1024; break;
			case MS_ZRTP_KEY_AGREEMENT_K448_HQC256:     bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K448_HQC256; break;
			case MS_ZRTP_KEY_AGREEMENT_K255_KYB512_HQC128:     bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K255_KYB512_HQC128; break;
			case MS_ZRTP_KEY_AGREEMENT_K448_KYB1024_HQC256:    bzrtpKeyAgreements[bzrtpCount++] = ZRTP_KEYAGREEMENT_K448_KYB1024_HQC256; break;
		}
	}
...
uint8_t ms_zrtp_available_key_agreement(MSZrtpKeyAgreement algos[256]) {
	...
	for (i=0; i < nbAlgos; i++) {
		switch (bzrtpAlgos[i]) {
			...
			case ZRTP_KEYAGREEMENT_HQC1: algos[count++] = MS_ZRTP_KEY_AGREEMENT_HQC1; break;
			case ZRTP_KEYAGREEMENT_HQC2: algos[count++] = MS_ZRTP_KEY_AGREEMENT_HQC2; break;
			case ZRTP_KEYAGREEMENT_HQC3: algos[count++] = MS_ZRTP_KEY_AGREEMENT_HQC3; break;
			case ZRTP_KEYAGREEMENT_K255_KYB512: algos[count++] = MS_ZRTP_KEY_AGREEMENT_K255_KYB512; break;
			case ZRTP_KEYAGREEMENT_K255_HQC128: algos[count++] = MS_ZRTP_KEY_AGREEMENT_K255_HQC128; break;
			case ZRTP_KEYAGREEMENT_K448_KYB1024: algos[count++] = MS_ZRTP_KEY_AGREEMENT_K448_KYB1024; break;
			case ZRTP_KEYAGREEMENT_K448_HQC256: algos[count++] = MS_ZRTP_KEY_AGREEMENT_K448_HQC256; break;
			case ZRTP_KEYAGREEMENT_K255_KYB512_HQC128: algos[count++] = MS_ZRTP_KEY_AGREEMENT_K255_KYB512_HQC128; break;
			case ZRTP_KEYAGREEMENT_K448_KYB1024_HQC256: algos[count++] = MS_ZRTP_KEY_AGREEMENT_K448_KYB1024_HQC256; break;
			default: break;
		}
	}
...
MS2_PUBLIC const char* ms_zrtp_auth_tag_to_string(const MSZrtpAuthTag authTag) {
	SWITCH_CRYPTO_ALGO(authTag,\
		...
		CASE_RETURN_STRING(MS_ZRTP_AUTHTAG_GCM);\
	);
}
...
const char* ms_zrtp_key_agreement_to_string(const MSZrtpKeyAgreement keyAgreement) {
	SWITCH_CRYPTO_ALGO(keyAgreement,\
		...
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_K255);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_K448);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_KYB1);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_KYB2);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_KYB3);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_HQC1);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_HQC2);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_HQC3);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_K255_KYB512);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_K255_HQC128);\
		CASE_RETURN_STRING(MS_ZRTP_KEY_AGREEMENT_K448_KYB1024);\
