...
struct _MSDtlsSrtpContext{
	...
	DtlsBcToolBoxContext *rtp_dtls_context; /**< a structure containing all contexts needed by DTLS handshake for RTP channel */
	DtlsBcToolBoxContext *rtcp_dtls_context; /**< a structure containing all contexts needed by DTLS handshake for RTCP channel */

	uint8_t rtp_channel_status; /**< channel status : DTLS_STATUS_CONTEXT_NOT_READY, DTLS_STATUS_CONTEXT_READY, DTLS_STATUS_HANDSHAKE_ONGOING, DTLS_STATUS_HANDSHAKE_OVER, DTLS_STATUS_FINGERPRINT_VERIFIED */
	uint8_t rtp_channel_key_material[DTLS_SRTP_KEY_MATERIAL_LEN]; /**< Store the key material generated by the handshake on rtp channel */
	MSCryptoSuite rtp_agreed_srtp_protection_profile; /**< agreed protection profile on rtp channel */

	uint8_t rtcp_channel_status; /**< channel status : DTLS_STATUS_CONTEXT_NOT_READY, DTLS_STATUS_CONTEXT_READY, DTLS_STATUS_HANDSHAKE_ONGOING, DTLS_STATUS_HANDSHAKE_OVER, DTLS_STATUS_FINGERPRINT_VERIFIED */
	uint8_t rtcp_channel_key_material[DTLS_SRTP_KEY_MATERIAL_LEN]; /**< Store the key material generated by the handshake on rtcp channel */
...
static void ms_dtls_srtp_set_srtp_key_material(MSDtlsSrtpContext *ctx, MSSrtpStreamType stream_type) {
	...
	if (ctx->role == MSDtlsSrtpRoleIsServer) {
		...
		ms_media_stream_sessions_set_srtp_recv_key(ctx->stream_sessions, srtp_protection_profile, (const char *)key, DTLS_SRTP_KEY_LEN+DTLS_SRTP_SALT_LEN, stream_type);
		...
		ms_media_stream_sessions_set_srtp_send_key(ctx->stream_sessions, srtp_protection_profile, (const char *)key, DTLS_SRTP_KEY_LEN+DTLS_SRTP_SALT_LEN, stream_type);
...
static int ms_dtls_srtp_rtp_process_on_receive(struct _RtpTransportModifier *t, mblk_t *msg){
	...
	if (ms_dtls_srtp_process_dtls_packet(msg, ctx, &ret, TRUE) == TRUE){
		if ((ret==0) && (ctx->rtp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING)) { /* handshake is over, give the keys to srtp : 128 bits client write - 128 bits server write - 112 bits client salt - 112 server salt */
			...
			ctx->rtp_agreed_srtp_protection_profile = ms_dtls_srtp_bctbx_protection_profile_to_ms_crypto_suite(bctbx_ssl_get_dtls_srtp_protection_profile(ctx->rtp_dtls_context->ssl));
			if ( ctx->rtp_agreed_srtp_protection_profile == MS_CRYPTO_SUITE_INVALID) {
				...
				ms_message("DTLS Handshake on RTP channel successful, srtp protection profile %d", ctx->rtp_agreed_srtp_protection_profile);

				ctx->rtp_time_reference = 0; /* unarm the timer */
				ret = bctbx_ssl_get_dtls_srtp_key_material(ctx->rtp_dtls_context->ssl_config, ctx->rtp_channel_key_material, &dtls_srt_key_material_length);
				if (ret < 0) {
...
=======
				ms_mutex_unlock(&ctx->rtp_dtls_context->ssl_context_mutex);
>>>>>>> 38832a31... DTLS SRTP manages handshake completed before the fingerprint arrival in SDP
			}
...
static int ms_dtls_srtp_rtcp_process_on_receive(struct _RtpTransportModifier *t, mblk_t *msg)  {
	...
	if (ms_dtls_srtp_process_dtls_packet(msg, ctx, &ret, FALSE) == TRUE){
		...
		if ((ret==0) && (ctx->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING)) { /* rtcp handshake is over, give the keys to srtp : 128 bits client write - 128 bits server write - 112 bits client salt - 112 server salt */
			...
			/* check the srtp profile get selected during handshake */
			ctx->rtcp_agreed_srtp_protection_profile = ms_dtls_srtp_bctbx_protection_profile_to_ms_crypto_suite(bctbx_ssl_get_dtls_srtp_protection_profile(ctx->rtcp_dtls_context->ssl));
			if ( ctx->rtcp_agreed_srtp_protection_profile == MS_CRYPTO_SUITE_INVALID) {
				...
				/* Get key material generated by DTLS handshake */
				size_t dtls_srt_key_material_length = DTLS_SRTP_KEY_MATERIAL_LEN;
				ms_message("DTLS RTCP Handshake successful, srtp protection profile %d", ctx->rtcp_agreed_srtp_protection_profile);

				ctx->rtcp_time_reference = 0; /* unarm the timer */

				ret = bctbx_ssl_get_dtls_srtp_key_material(ctx->rtcp_dtls_context->ssl_config, ctx->rtcp_channel_key_material, &dtls_srt_key_material_length);
				if (ret < 0) {
...
void ms_dtls_srtp_set_peer_fingerprint(MSDtlsSrtpContext *ctx, const char *peer_fingerprint) {
	if (ctx) {
		...
		if (peer_fingerprint_length>sizeof(ctx->peer_fingerprint)) {
			...
			ms_error("DTLS-SRTP received from SDP INVITE a peer fingerprint %d bytes length wich is longer than maximum storage %d bytes", (int)peer_fingerprint_length, (int)sizeof(ctx->peer_fingerprint));
		} else {
