...
static bool_t ms_dtls_srtp_process_dtls_packet(mblk_t *msg, MSDtlsSrtpContext *ctx, int *ret) {
	...
	uint64_t *time_reference = &(ctx->rtp_time_reference);
	bctbx_ssl_context_t *ssl = ctx->rtp_dtls_context->ssl;
	ms_mutex_t *mutex = &ctx->rtp_dtls_context->ssl_context_mutex;
	...
	if ((*(msg->b_rptr)>19) && (*(msg->b_rptr)<64)) {
		...
		ms_message("DTLS Receive RTP packet len %d sessions: %p rtp session %p", (int)msgLength, ctx->stream_sessions, ctx->stream_sessions->rtp_session);
				...
				if (frag[Content_Type_Index] == 0x16) { // Type index 0x16 is DLTS Handshake message
					if (frag[Handshake_Type_Index] == 0x01) { // Handshake type 0x01 is Client Hello
						...
						if ( base_index + Handshake_Header_Length + frag_length <= msgLength // we will read frag_length starting at base_index (frag in the code)+ header
							...
							ms_warning("DTLS Received RTP packet len %d sessions: %p rtp session %p is malformed in an agressive way", (int)msgLength, ctx->stream_sessions, ctx->stream_sessions->rtp_session);
		...
		if (channel_status < DTLS_STATUS_HANDSHAKE_OVER) {
			...
			if (ctx->role == MSDtlsSrtpRoleUnset) {
				...
				ms_dtls_srtp_start(ctx); /* complete the ssl setup and change channel_status to DTLS_STATUS_HANDSHAKE_ONGOING */
			}
			ms_mutex_lock(mutex);
			/* process the packet and store result */
			*ret = bctbx_ssl_handshake(ssl);
			ms_message("DTLS Handshake process RTP packet len %d sessions: %p rtp session %p return %s0x%0x", (int)msgLength, ctx->stream_sessions, ctx->stream_sessions->rtp_session, *ret>0?"+":"-", *ret>0?*ret:-*ret);
			...
			*ret = bctbx_ssl_read(ssl, buf, msgLength);
			ms_message("DTLS Handshake read RTP packet len %d sessions: %p rtp session %p return %s0x%0x", (int)msgLength, ctx->stream_sessions, ctx->stream_sessions->rtp_session, *ret>0?"+":"-", *ret>0?*ret:-*ret);
			ms_free(buf);
			ms_mutex_unlock(mutex);
		}
		...
		if (*ret<0 && *ret != BCTBX_ERROR_NET_WANT_READ) {
			...
			ms_warning("DTLS Handshake returns -0x%x : %s [on sessions: %p rtp session %p]", -*ret, err_str, ctx->stream_sessions, ctx->stream_sessions->rtp_session);
		}
 ...
 */
static void ms_dtls_srtp_set_srtp_key_material(MSDtlsSrtpContext *ctx) {
	...
	if (ctx->role == MSDtlsSrtpRoleIsServer) {
		...
		ms_media_stream_sessions_set_srtp_recv_key(ctx->stream_sessions, srtp_protection_profile, key, DTLS_SRTP_KEY_LEN+DTLS_SRTP_SALT_LEN, MSSrtpKeySourceDTLS);
		...
		ms_media_stream_sessions_set_srtp_send_key(ctx->stream_sessions, srtp_protection_profile, key, DTLS_SRTP_KEY_LEN+DTLS_SRTP_SALT_LEN, MSSrtpKeySourceDTLS);
...
MSDtlsSrtpContext* ms_dtls_srtp_context_new(MSMediaStreamSessions *sessions, MSDtlsSrtpParams *params) {
	...
	ms_message("Creating DTLS-SRTP engine on stream sessions [%p] as %s, RTCP mux is %s", sessions, params->role==MSDtlsSrtpRoleIsServer?"server":(params->role==MSDtlsSrtpRoleIsClient?"client":"unset role"), rtp_session_rtcp_mux_enabled(s)?"enabled":"disabled");
...
void ms_dtls_srtp_start(MSDtlsSrtpContext* context) {
	...
	if (context->role == MSDtlsSrtpRoleIsServer) {
		if (context->rtp_channel_status == DTLS_STATUS_CONTEXT_READY) {
			...
			context->rtp_channel_status = DTLS_STATUS_HANDSHAKE_ONGOING;
			ms_mutex_unlock(&context->rtp_dtls_context->ssl_context_mutex);
		} else if (context->rtp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING) {
			/* Handshake was already ongoing, probably started by an incoming Client Hello, we might have data to be sent */
			bctbx_ssl_handshake(context->rtp_dtls_context->ssl);
		}
	}
