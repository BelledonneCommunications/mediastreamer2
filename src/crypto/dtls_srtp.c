	...
	if ((*(msg->b_rptr) > 19) && (*(msg->b_rptr) < 64)) {
		...
		if (channel_status < DTLS_STATUS_HANDSHAKE_OVER) {
			...
			*ret = bctbx_ssl_read(ssl, buf, msgLength);
			ms_message("DTLS Handshake read RTP packet len %d sessions: %p rtp session %p return %s0x%0x",
			           (int)msgLength, ctx->stream_sessions, ctx->stream_sessions->rtp_session, *ret > 0 ? "+" : "-",
			           *ret > 0 ? *ret : -*ret);
			ms_free(buf);
			ms_mutex_unlock(mutex);
...
static int ms_dtls_srtp_initialise_bctbx_dtls_context(DtlsBcToolBoxContext *dtlsContext,
...
void ms_dtls_srtp_start(MSDtlsSrtpContext *context) {
	...
	if (context->role == MSDtlsSrtpRoleIsServer) {
		if (context->rtp_channel_status == DTLS_STATUS_CONTEXT_READY) {
			...
			/* Handshake was already ongoing, probably started by an incoming Client Hello, we might have data to be
			 * sent */
			bctbx_ssl_handshake(context->rtp_dtls_context->ssl);
		}
	}
