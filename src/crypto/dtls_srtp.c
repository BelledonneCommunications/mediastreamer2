...
struct _MSDtlsSrtpContext {
	...
	DtlsBcToolBoxContext
		*rtcp_dtls_context; /**< a structure containing all contexts needed by DTLS handshake for RTCP channel */
	...
	uint8_t rtcp_channel_status; /**< channel status : DTLS_STATUS_CONTEXT_NOT_READY, DTLS_STATUS_CONTEXT_READY,
									DTLS_STATUS_HANDSHAKE_ONGOING, DTLS_STATUS_HANDSHAKE_OVER,
									DTLS_STATUS_FINGERPRINT_VERIFIED */
	uint8_t rtcp_channel_key_material[DTLS_SRTP_KEY_MATERIAL_LEN]; /**< Store the key material generated by the
																	  handshake on rtcp channel */
	MSCryptoSuite rtcp_agreed_srtp_protection_profile;			   /**< agreed protection profile on rtcp channel */

	DtlsRawPacket *rtp_incoming_buffer;	 /**< buffer of incoming DTLS packet to be read by mbedtls callback */
	DtlsRawPacket *rtcp_incoming_buffer; /**< buffer of incoming DTLS packet to be read by mbedtls callback */
 ...
 */
static uint8_t ms_dtls_srtp_check_certificate_fingerprint(const bctbx_x509_certificate_t *certificate,
 ...
 * @param[out]		ret	the value returned by the bctoolbox function processing the packet(ssl_handshake)
...
static bool_t ms_dtls_srtp_process_dtls_packet(mblk_t *msg, MSDtlsSrtpContext *ctx, int *ret, bool_t is_rtp) {
	...
	ms_mutex_t *mutex =
		(is_rtp == TRUE) ? &ctx->rtp_dtls_context->ssl_context_mutex : &ctx->rtcp_dtls_context->ssl_context_mutex;
	...
	if ((*(msg->b_rptr) > 19) && (*(msg->b_rptr) < 64)) {
		...
		ms_message("DTLS Receive %s packet len %d sessions: %p rtp session %p", is_rtp == TRUE ? "RTP" : "RTCP",
				...
				if (frag[Content_Type_Index] == 0x16) {		  // Type index 0x16 is DLTS Handshake message
					if (frag[Handshake_Type_Index] == 0x01) { // Handshake type 0x01 is Client Hello
						...
						if (base_index + Handshake_Header_Length + frag_length <=
							...
							ms_warning("DTLS Received %s packet len %d sessions: %p rtp session %p is malformed in an "
									   "agressive way",
									   is_rtp == TRUE ? "RTP" : "RTCP", (int)msgLength, ctx->stream_sessions,
		...
		if (channel_status < DTLS_STATUS_HANDSHAKE_OVER) {
			...
			*ret = bctbx_ssl_handshake(ssl);
			ms_message("DTLS Handshake process %s packet len %d sessions: %p rtp session %p return %s0x%0x",
					   is_rtp == TRUE ? "RTP" : "RTCP", (int)msgLength, ctx->stream_sessions,
					   ctx->stream_sessions->rtp_session, *ret > 0 ? "+" : "-", *ret > 0 ? *ret : -*ret);
			...
			*ret = bctbx_ssl_read(ssl, buf, msgLength);
			ms_message("DTLS Handshake read %s packet len %d sessions: %p rtp session %p return %s0x%0x",
					   is_rtp == TRUE ? "RTP" : "RTCP", (int)msgLength, ctx->stream_sessions,
					   ctx->stream_sessions->rtp_session, *ret > 0 ? "+" : "-", *ret > 0 ? *ret : -*ret);
			ms_free(buf);
			ms_mutex_unlock(mutex);
		...
		if (*ret < 0 && *ret != BCTBX_ERROR_NET_WANT_READ) {
			...
			ms_warning("DTLS handhake returns -0x%x : %s [on sessions: %p rtp session %p]", -*ret, err_str,
...
static void ms_dtls_srtp_check_channels_status(MSDtlsSrtpContext *ctx) {
	...
	if (((ctx->rtp_channel_status == DTLS_STATUS_FINGERPRINT_VERIFIED) &&
		...
		((ctx->rtp_channel_status == DTLS_STATUS_FINGERPRINT_VERIFIED) &&
		 (ctx->rtcp_channel_status == DTLS_STATUS_FINGERPRINT_VERIFIED))) {
 ...
 * @param	stream_type	MSSRTP_RTP_STREAM or MSSRTP_RTCP_STREAM
 */
static void ms_dtls_srtp_set_srtp_key_material(MSDtlsSrtpContext *ctx, MSSrtpStreamType stream_type) {

	uint8_t key[256];
	MSCryptoSuite srtp_protection_profile = MS_CRYPTO_SUITE_INVALID;
	uint8_t *key_material;

	if (stream_type == MSSRTP_RTP_STREAM) {
		srtp_protection_profile = ctx->rtp_agreed_srtp_protection_profile;
		key_material = ctx->rtp_channel_key_material;
	} else if (stream_type == MSSRTP_RTCP_STREAM) {
		srtp_protection_profile = ctx->rtcp_agreed_srtp_protection_profile;
		key_material = ctx->rtcp_channel_key_material;
	} else {
		ms_error("DTLS SRTP cannot set srtp context key material in unknown stream type %d", stream_type);
		return;
	}
	...
	if (ctx->role == MSDtlsSrtpRoleIsServer) {
		...
		ms_media_stream_sessions_set_srtp_recv_key(ctx->stream_sessions, srtp_protection_profile, (const char *)key,
												   DTLS_SRTP_KEY_LEN + DTLS_SRTP_SALT_LEN, stream_type);
		...
		ms_media_stream_sessions_set_srtp_send_key(ctx->stream_sessions, srtp_protection_profile, (const char *)key,
												   DTLS_SRTP_KEY_LEN + DTLS_SRTP_SALT_LEN, stream_type);
...
static int ms_dtls_srtp_rtcp_sendData(void *ctx, const unsigned char *data, size_t length) {
	...
	ms_message("DTLS Send RTCP packet len %d sessions: %p rtp session %p", (int)length, context->stream_sessions,
			   context->stream_sessions->rtp_session);
	...
	if (ms_dtls_srtp_process_dtls_packet(msg, ctx, &ret, TRUE) == TRUE) {
		if ((ret == 0) &&
			...
			if (ctx->rtp_agreed_srtp_protection_profile == MS_CRYPTO_SUITE_INVALID) {
				...
				size_t dtls_srt_key_material_length = DTLS_SRTP_KEY_MATERIAL_LEN;
				...
				ret = bctbx_ssl_get_dtls_srtp_key_material(
					ctx->rtp_dtls_context->ssl_config, ctx->rtp_channel_key_material, &dtls_srt_key_material_length);
				if (ret < 0) {
				...
				ms_mutex_lock(&ctx->rtp_dtls_context->ssl_context_mutex); /* fingerprint can be written by incoming SIP
																			 packet in an other thread */
				...
				if (ms_dtls_srtp_check_certificate_fingerprint(
						bctbx_ssl_get_peer_certificate(ctx->rtp_dtls_context->ssl),
						(const char *)(ctx->peer_fingerprint)) == 1) {
					ms_dtls_srtp_set_srtp_key_material(ctx, MSSRTP_RTP_STREAM);
					ctx->rtp_channel_status = DTLS_STATUS_FINGERPRINT_VERIFIED;
...
static int ms_dtls_srtp_rtcp_process_on_receive(struct _RtpTransportModifier *t, mblk_t *msg) {
	...
	if (ms_dtls_srtp_process_dtls_packet(msg, ctx, &ret, FALSE) == TRUE) {
		...
		if ((ret == 0) &&
			(ctx->rtcp_channel_status ==
			 DTLS_STATUS_HANDSHAKE_ONGOING)) { /* rtcp handshake is over, give the keys to srtp : 128 bits client write
												  - 128 bits server write - 112 bits client salt - 112 server salt */
			ctx->rtcp_channel_status = DTLS_STATUS_HANDSHAKE_OVER;

			/* check the srtp profile get selected during handshake */
			ctx->rtcp_agreed_srtp_protection_profile = ms_dtls_srtp_bctbx_protection_profile_to_ms_crypto_suite(
				bctbx_ssl_get_dtls_srtp_protection_profile(ctx->rtcp_dtls_context->ssl));
			if (ctx->rtcp_agreed_srtp_protection_profile == MS_CRYPTO_SUITE_INVALID) {
				ms_error("DTLS RTCP handshake successful but unable to agree on srtp_profile to use");
				return 0;
			} else {
				/* Get key material generated by DTLS handshake */
				size_t dtls_srt_key_material_length = DTLS_SRTP_KEY_MATERIAL_LEN;
				ms_message("DTLS RTCP Handshake successful, srtp protection profile %d",
						   ctx->rtcp_agreed_srtp_protection_profile);

				ctx->rtcp_time_reference = 0; /* unarm the timer */

				ret = bctbx_ssl_get_dtls_srtp_key_material(
					ctx->rtcp_dtls_context->ssl_config, ctx->rtcp_channel_key_material, &dtls_srt_key_material_length);
				if (ret < 0) {
				...
				/* Check certificate fingerprint */
				ms_mutex_lock(&ctx->rtcp_dtls_context->ssl_context_mutex); /* fingerprint can be written by incoming SIP
																			  packet in an other thread */
				if (ctx->peer_fingerprint[0] == '\0') { /* fingerprint not set yet - peer's 200Ok didn't arrived yet */
				...
				if (ms_dtls_srtp_check_certificate_fingerprint(
						bctbx_ssl_get_peer_certificate(ctx->rtcp_dtls_context->ssl),
						(const char *)(ctx->peer_fingerprint)) == 1) {
					ms_dtls_srtp_set_srtp_key_material(ctx, MSSRTP_RTCP_STREAM);
					ctx->rtcp_channel_status = DTLS_STATUS_FINGERPRINT_VERIFIED;
					ms_dtls_srtp_check_channels_status(ctx);
				}
				ms_mutex_unlock(&ctx->rtcp_dtls_context->ssl_context_mutex);
			...
			if (ctx->role != MSDtlsSrtpRoleIsServer) { /* close the connection only if we are client, if we are server,
														  the client may ask again for last packets */
				/*FireFox version 43 requires DTLS channel to be kept openned, probably a bug in FireFox  ret =
				 * ssl_close_notify( &(ctx->rtcp_dtls_context->ssl) );*/
			}
		}
...
static int ms_dtls_srtp_transport_modifier_new(MSDtlsSrtpContext *ctx, RtpTransportModifier **rtpt,
											   RtpTransportModifier **rtcpt) {
	if (rtpt) {
	...
	if (rtcpt) {
		*rtcpt = ms_new0(RtpTransportModifier, 1);
		(*rtcpt)->data = ctx; /* back link to get access to the other fields of the OrtoZrtpContext from the
								 RtpTransportModifier structure */
...
void ms_dtls_srtp_set_peer_fingerprint(MSDtlsSrtpContext *ctx, const char *peer_fingerprint) {
	if (ctx) {
		/* Finishing handshake can be checking the fingerprint from an other thread */
		ms_mutex_lock(&ctx->rtp_dtls_context->ssl_context_mutex);
		ms_mutex_lock(&ctx->rtcp_dtls_context->ssl_context_mutex);
		...
		if (ctx->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_OVER) {
			ms_message("DTLS SRTP : late fingerprint arrival, check it after RTCP Handshake is over");
			if (ms_dtls_srtp_check_certificate_fingerprint(bctbx_ssl_get_peer_certificate(ctx->rtcp_dtls_context->ssl),
														   (const char *)(ctx->peer_fingerprint)) == 1) {
				ms_dtls_srtp_set_srtp_key_material(ctx, MSSRTP_RTCP_STREAM);
				ctx->rtcp_channel_status = DTLS_STATUS_FINGERPRINT_VERIFIED;
				ms_dtls_srtp_check_channels_status(ctx);
...
void ms_dtls_srtp_reset_context(MSDtlsSrtpContext *context) {
	if (context) {
		...
		if ((context->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_ONGOING) ||
			(context->rtcp_channel_status == DTLS_STATUS_HANDSHAKE_OVER)) {
			bctbx_ssl_session_reset(context->rtcp_dtls_context->ssl);
		}

		context->rtcp_channel_status = DTLS_STATUS_CONTEXT_READY;
		context->rtcp_agreed_srtp_protection_profile = MS_CRYPTO_SUITE_INVALID;
...
void ms_dtls_srtp_set_role(MSDtlsSrtpContext *context, MSDtlsSrtpRole role) {
	if (context) {
		...
		ms_message("DTLS set role from [%s] to [%s] for context [%p]",
				   ...
				   role == MSDtlsSrtpRoleIsServer ? "server"
												  : (role == MSDtlsSrtpRoleIsClient ? "client" : "unset role"),
				   context);
		context->role = role;
		ms_mutex_unlock(&context->rtp_dtls_context->ssl_context_mutex);
		ms_mutex_unlock(&context->rtcp_dtls_context->ssl_context_mutex);
...
MSDtlsSrtpContext *ms_dtls_srtp_context_new(MSMediaStreamSessions *sessions, MSDtlsSrtpParams *params) {
	...
	ms_message("Creating DTLS-SRTP engine on session [%p] as %s", s,
