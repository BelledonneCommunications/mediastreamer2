...
static void sender_init(MSFilter * f)
{
	...
	d->mixer_to_client_send_interval = 260;
	d->mixer_to_client_extension_id = RTP_EXTENSION_MIXER_TO_CLIENT_AUDIO_LEVEL;
	...
	do {
		...
		if (im){
			...
			if (d->skip == FALSE && d->mute==FALSE){
				if (f->ticker->time % d->mixer_to_client_send_interval == 0 && d->mtc_request_data_cb) {
