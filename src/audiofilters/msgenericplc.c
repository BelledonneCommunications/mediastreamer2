...
static void generic_plc_process(MSFilter *f) {
	...
	while((m=ms_queue_get(f->inputs[0]))!=NULL){
		...
		if (plc_context->plc_samples_used!=0) {
			 ...
			 * The first transitionBufferSize bytes of b_rptr have been written from continuity buffer, current incoming message starts at
			 * b_rptr+transitionBufferSize. However if the incoming message is shorter than 2*continuity buffer, there is no data to mix as
			 * it was swallowed by the continuity buffer. */
			if (msg_size >= (size_t)(2*transitionBufferSize)) {
				...
				/* new incoming audio is not in b_rptr but at the begining of the continuity buffer, so we mix the end of it with its beginning */
				...
				generic_plc_transition_mix((int16_t *)(plc_context->continuity_buffer), (int16_t *)(plc_context->continuity_buffer+transitionBufferSize), mgps->rate*TRANSITION_DELAY/1000);
			}
		}
