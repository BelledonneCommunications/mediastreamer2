...
-(void)create_audio_unit {
	...
	bool_t noVoiceProc = bctbx_param_string_get_bool_value(_ms_snd_card->sndcardmanager->paramString, MS_SND_CARD_MANAGER_PARAM_NOVOICEPROC) || _ms_snd_card->streamType == MS_SND_CARD_STREAM_MEDIA;
...
-(void)destroy_audio_unit {
	...
	if (_audio_unit) {
		...
		if ( !bctbx_param_string_get_bool_value(_ms_snd_card->sndcardmanager->paramString, MS_SND_CARD_MANAGER_PARAM_FAST) ) {
			NSError *err = nil;;
...
-(void) configure_audio_session {
	...
	if ( bctbx_param_string_get_bool_value(_ms_snd_card->sndcardmanager->paramString, MS_SND_CARD_MANAGER_PARAM_FAST) ) {
		...
		}else if (bctbx_param_string_get_bool_value(_ms_snd_card->sndcardmanager->paramString, MS_SND_CARD_MANAGER_PARAM_RINGER)){
...
static void check_unused(){
	...
	if ( bctbx_param_string_get_bool_value([au_holder ms_snd_card]->sndcardmanager->paramString, MS_SND_CARD_MANAGER_PARAM_TESTER) || ![au_holder will_be_used] ) {
		[au_holder stop_audio_unit];
		[au_holder destroy_audio_unit];
	}
}
...
static void au_write_process(MSFilter *f){
	...
	char* cardname = [[AudioUnitHolder sharedInstance] ms_snd_card]->name;
	ms_mutex_lock(&d->mutex);
