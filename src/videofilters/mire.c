...
void mire_init(MSFilter *f){
	...
	ms_message("[mire]ppppp init %p %d",d, d->colors[3]);
	...
	//d->colors = {150,12,100,60,200,100};
	//d->colors = tempColors;
}
...
static void mire_draw(MireData *d){
	//ms_message("[mire]llllll%p   %d, %d,%d,%d,%d,%d",d, d->colors[0], d->colors[1], d->colors[2], d->colors[3], d->colors[4], d->colors[5]);
...
static int mire_set_color(MSFilter *f, void* data){
	...
	//ms_message("[mire]kkkkkkk%p   %d, %d,%d,%d,%d,%d",d, d->colors[0], d->colors[1], d->colors[2], d->colors[3], d->colors[4], d->colors[5]);
	return 0;
}
...
MSFilterMethod mire_methods[]={
	...
	{	MS_FILTER_SET_COLOR	, mire_set_color},
	{	0,0 }
};
