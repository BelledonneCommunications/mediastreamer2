...
#define MS_VIDEOENC_CONF(required_bitrate, bitrate_limit, resolution, fps, cpu, qminvalue)                             \
	{                                                                                                                  \
		required_bitrate, bitrate_limit, {MS_VIDEO_SIZE_##resolution##_W, MS_VIDEO_SIZE_##resolution##_H}, fps, cpu,   \
		    (void *)&qmin##qminvalue                                                                                   \
	}
...
static const MSVideoConfiguration h263_conf_list[] = {
    MS_VIDEOENC_CONF(800000, 1024000, 4CIF, 25, 2, 2), MS_VIDEOENC_CONF(512000, 800000, CIF, 25, 1, 2),
    MS_VIDEOENC_CONF(256000, 512000, CIF, 17, 1, 3), MS_VIDEOENC_CONF(128000, 256000, QCIF, 10, 1, 3),
...
static const MSVideoConfiguration mjpeg_conf_list[] = {
    MS_VIDEOENC_CONF(1024000, 1536000, SVGA, 25, 1, 2), MS_VIDEOENC_CONF(800000, 1024000, VGA, 25, 1, 2),
    MS_VIDEOENC_CONF(512000, 800000, CIF, 25, 1, 2),    MS_VIDEOENC_CONF(256000, 512000, CIF, 17, 1, 3),
    MS_VIDEOENC_CONF(170000, 256000, QVGA, 15, 1, 3),   MS_VIDEOENC_CONF(128000, 170000, QCIF, 10, 1, 3),
    MS_VIDEOENC_CONF(0, 128000, QCIF, 5, 1, 5)};

static const MSVideoConfiguration mpeg4_conf_list[] = {
    MS_VIDEOENC_CONF(2048000, 2560000, SXGA_MINUS, 25, 4, 2), MS_VIDEOENC_CONF(2048000, 2560000, 720P, 25, 4, 2),
    MS_VIDEOENC_CONF(1536000, 2048000, XGA, 25, 4, 2),        MS_VIDEOENC_CONF(1024000, 1536000, SVGA, 25, 2, 2),
    MS_VIDEOENC_CONF(800000, 1024000, VGA, 25, 1, 2),         MS_VIDEOENC_CONF(512000, 800000, CIF, 25, 1, 2),
    MS_VIDEOENC_CONF(256000, 512000, CIF, 17, 1, 3),          MS_VIDEOENC_CONF(170000, 256000, QVGA, 15, 1, 3),
    MS_VIDEOENC_CONF(128000, 170000, QCIF, 10, 1, 3),         MS_VIDEOENC_CONF(0, 128000, QCIF, 5, 1, 5)};

#if HAVE_AVCODEC_SNOW
static const MSVideoConfiguration snow_conf_list[] = {
    MS_VIDEOENC_CONF(1024000, 1536000, SVGA, 25, 2, 2), MS_VIDEOENC_CONF(800000, 1024000, VGA, 25, 1, 2),
    MS_VIDEOENC_CONF(512000, 800000, CIF, 25, 1, 2),    MS_VIDEOENC_CONF(256000, 512000, CIF, 17, 1, 3),
    MS_VIDEOENC_CONF(170000, 256000, QVGA, 15, 1, 3),   MS_VIDEOENC_CONF(128000, 170000, QCIF, 10, 1, 3),
    MS_VIDEOENC_CONF(64000, 128000, QCIF, 7, 1, 4),     MS_VIDEOENC_CONF(0, 64000, QCIF, 5, 1, 5)};
#endif
...
static void enc_preprocess(MSFilter *f) {
	...
	int error;
	prepare(s);
	if (s->codec == CODEC_ID_H263P || s->codec == CODEC_ID_H263) prepare_h263(s);
	else if (s->codec == CODEC_ID_MPEG4) prepare_mpeg4(s);
#if HAVE_AVCODEC_SNOW
...
static int get_gbsc_bytealigned(uint8_t *begin, uint8_t *end) {
	...
	for (i = len - 2; /*len + length of scan window*/
	     ...
	     i--) {
		if (*(begin + i) == 0 && *(begin + i + 1) == 0 && (*(begin + i + 2) & 0x80) == 0x80) {
...
static void rfc4629_generate_follow_on_packets(
    MSFilter *f, EncState *s, mblk_t *frame, uint32_t timestamp, uint8_t *psc, uint8_t *end, bool_t last_packet) {
	mblk_t *packet;
	int len = (int)(end - psc);
...
static void mjpeg_fragment_and_send(MSFilter *f,
                                    EncState *s,
                                    mblk_t *frame,
                                    uint32_t timestamp,
                                    uint8_t type,
...
static void split_and_send(MSFilter *f, EncState *s, mblk_t *frame, bool_t is_iframe) {
	...
	if (s->codec == CODEC_ID_MPEG4
		...
		mjpeg_fragment_and_send(f, s, frame, timestamp, 1, /* 420? */
		                        0, 0,                      /* dri ?*/
		                        255,                       /* q */
		                        lqt, cqt);
		return;
...
static int enc_set_configuration(MSFilter *f, void *data) {
	...
	ms_message("Video configuration set: bitrate=%dbits/s, fps=%f, vsize=%dx%d for encoder [%p]",
...
static MSFilterMethod methods[] = {{MS_FILTER_ADD_FMTP, enc_add_fmtp},
...
MSFilterDesc ms_h263_enc_desc = {MS_H263_ENC_ID,
                                 "MSH263Enc",
...
MSFilterDesc ms_mpeg4_enc_desc = {MS_MPEG4_ENC_ID,
                                  "MSMpeg4Enc",
                                  ...
                                  methods};
#if HAVE_AVCODEC_SNOW
MSFilterDesc ms_snow_enc_desc = {MS_SNOW_ENC_ID,
                                 "MSSnowEnc",
                                 ...
                                 methods};
#endif
MSFilterDesc ms_mjpeg_enc_desc = {MS_JPEG_ENC_ID,
                                  "MSJpegEnc",
...
MSFilterDesc ms_h263_enc_desc = {.id = MS_H263_ENC_ID,
                                 .name = "MSH263Enc",
...
MSFilterDesc ms_mpeg4_enc_desc = {.id = MS_MPEG4_ENC_ID,
                                  .name = "MSMpeg4Enc",
...
MSFilterDesc ms_snow_enc_desc = {
    ...
    .methods = methods};
#endif
MSFilterDesc ms_mjpeg_enc_desc = {.id = MS_JPEG_ENC_ID,
                                  .name = "MSMJpegEnc",
...
#if HAVE_AVCODEC_SNOW
	if (avcodec_find_encoder(CODEC_ID_SNOW)) ms_factory_register_filter(obj, &ms_snow_enc_desc);
#endif
