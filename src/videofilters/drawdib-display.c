...
static void yuv2rgb_prepare(Yuv2RgbCtx *ctx, MSVideoSize src, MSVideoSize dst) {
	...
	ctx->sws = ms_scaler_create_context(src.width, src.height, MS_YUV420P, dst.width, dst.height, MS_RGB24_REV,
...
static void yuv2rgb_draw(Yuv2RgbCtx *ctx, HDRAWDIB ddh, HDC hdc, int dstx, int dsty) {
	if (ctx->rgb) {
		...
		DrawDibDraw(ddh, hdc, dstx, dsty, -1, -1, &bi, ctx->rgb, 0, 0, ctx->dsize.width, ctx->dsize.height, 0);
	}
}
...
static LRESULT CALLBACK window_proc(HWND hwnd,     // handle to window
...
#ifdef MS2_WINDOWS_UWP
	hwnd = CreateWindow(L"Video Window", L"Video window", WS_OVERLAPPEDWINDOW /*WS_THICKFRAME*/ | WS_VISIBLE,
	                    CW_USEDEFAULT, CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top, NULL, NULL,
	                    hInstance, dd);
#else
	hwnd =
	    CreateWindow("Video Window", "Video window", WS_OVERLAPPEDWINDOW /*WS_THICKFRAME*/ | WS_VISIBLE, CW_USEDEFAULT,
	                 CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top, NULL, NULL, hInstance, dd);
#endif
...
static void dd_display_prepare(MSFilter *f) {
	...
	if (dd->ddh == NULL) dd->ddh = DrawDibOpen();
	// do not automatically resize video window if we don't created it.
	if (dd->own_window == FALSE) dd->autofit = FALSE;
}
...
static void dd_display_process(MSFilter *f) {
	...
	/* this creates the window if not given by the application. This must be done within the process() function because
	 ...
	 Previously, window creation was done from preprocess() (so main thread) but it was deadlocking when process()
	 function was doing MoveWindow() to resize the window*/
	...
	if (GetClientRect(obj->window, &rect) == 0 || rect.right <= 32 || rect.bottom <= 32) goto end;

	wsize.width = rect.right;
	wsize.height = rect.bottom;
	if (!ms_video_size_equal(wsize, obj->wsize)) obj->need_repaint = TRUE;
	obj->wsize = wsize;
	...
	if (main_im != NULL || local_im != NULL || obj->need_repaint) {
		...
		if (local_im != NULL) yuv2rgb_process(&obj->locview, &localpic, lsize, !mblk_get_precious_flag(local_im));
		...
		if (obj->locview.rgb == NULL) {
			...
			hdc2 = hdc;
			if (obj->need_repaint) draw_background(hdc2, wsize, mainrect, obj->background_color);
		} else {
...
static int set_scalefactor(MSFilter *f, void *arg) {
	...
	if (obj->sv_scalefactor < 0.5f) obj->sv_scalefactor = 0.5f;
	ms_filter_unlock(f);
	return 0;
}
...
static MSFilterMethod methods[] = {{MS_FILTER_GET_VIDEO_SIZE, get_vsize},
...
MSFilterDesc ms_dd_display_desc = {MS_DRAWDIB_DISPLAY_ID,
                                   "MSDrawDibDisplay",
...
MSFilterDesc ms_dd_display_desc = {.id = MS_DRAWDIB_DISPLAY_ID,
