...
static int analysedisplay_compare_color(MSFilter *f, void* data) {
	...
	MSAnalyseDisplayData* d = (MSAnalyseDisplayData*)f->data;
	if (!d->pict.planes[0]) {
		ms_error("[MSAnalyseDisplay] can not get picture. Please check if the process is called.");
		ms_filter_unlock(f);
		return -1;
	}

	int source = d->pict.planes[0][0];
	int dest = *(int *)data;
	ms_message("[MSAnalyseDisplay] compare source color %d with wanted color %d",source,dest);
	ms_filter_unlock(f);
...
static MSFilterMethod analysedisplay_methods[] = {
	{ MS_VIDEO_DISPLAY_COMPARE_COLOR, analysedisplay_compare_color},
	{ 0, NULL }
};
