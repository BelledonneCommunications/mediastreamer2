...
static inline uint64_t _obp_br_unchecked(_OBPBitReader *br, uint8_t n) {
	...
	while (n > br->bits_in_buf) {
		...
		if (br->bits_in_buf > 56) {
			if (n <= br->bits_in_buf) break;

			if (n <= 64) return (_obp_br_unchecked(br, 32) << 32) | (_obp_br_unchecked(br, n - 32));
		}
	}
...
static inline int32_t _obp_get_relative_dist(int32_t a, int32_t b, OBPSequenceHeader *seq) {
	...
	if (!seq->enable_order_hint) return 0;
...
static inline int
_obp_decode_unsigned_subexp_with_ref(_OBPBitReader *br, int32_t mx, int32_t r, int16_t *out, OBPError *err) {
	uint32_t v;
...
static inline int _obp_decode_signed_subexp_with_ref(
    _OBPBitReader *br, int32_t low, int32_t high, int32_t r, int16_t *out, OBPError *err) {
	int16_t val;
...
static inline int _obp_is_valid_obu(OBPOBUType type) {
	return type == OBP_OBU_SEQUENCE_HEADER || type == OBP_OBU_TEMPORAL_DELIMITER || type == OBP_OBU_FRAME_HEADER ||
	       type == OBP_OBU_TILE_GROUP || type == OBP_OBU_METADATA || type == OBP_OBU_FRAME ||
	       type == OBP_OBU_REDUNDANT_FRAME_HEADER || type == OBP_OBU_TILE_LIST || type == OBP_OBU_PADDING;
}
...
static inline int _obp_set_frame_refs(OBPFrameHeader *fh, OBPSequenceHeader *seq, OBPState *state, OBPError *err) {
	...
	const int Ref_Frame_List[7 - 2] = {2, 3, 5, 6,
	                                   7}; /*LAST2_FRAME, LAST3_FRAME, BWDREF_FRAME, ALTREF2_FRAME, ALTREF_FRAME */
	...
	if (lastOrderHint >= curFrameHint || goldOrderHint >= curFrameHint) {
		snprintf(err->error, err->size,
		         "(lastOrderHint >= curFrameHint || goldOrderHint >= curFrameHint) not allowed.");
		return -1;
	}
...
static inline uint8_t _obp_get_qindex(int ignoreDeltaQ,
                                      int segmentId,
                                      int16_t CurrentQIndex,
                                      OBPFrameHeader *fh,
...
static inline int
_obp_read_global_param(_OBPBitReader *br, OBPFrameHeader *fh, uint8_t type, int ref, int idx, OBPError *err) {
	uint8_t absBits = 12;
	uint8_t precBits = 15;
	if (idx < 2) {
		if (type == 1) {
...
int obp_get_next_obu(uint8_t *buf,
                     size_t buf_size,
                     OBPOBUType *obu_type,
                     ptrdiff_t *offset,
                     size_t *size,
...
int obp_parse_tile_list(uint8_t *buf, size_t buf_size, OBPTileList *tile_list, OBPError *err) {
	...
	for (uint16_t i = 0; i < tile_list->tile_count_minus_1; i++) {
		...
		if (pos + N > buf_size) {
			snprintf(err->error, err->size,
			         "Tile list OBU malformed: Not enough bytes for next tile_list_entry()'s data.");
			return -1;
		}
...
int obp_parse_tile_group(uint8_t *buf,
                         size_t buf_size,
	...
	if (tile_group->NumTiles == 1 || !tile_group->tile_start_and_end_present_flag) {
		...
		uint8_t tileBits =
		    _obp_tile_log2(1, frame_header->tile_info.TileCols) + _obp_tile_log2(1, frame_header->tile_info.TileRows);
	...
	for (uint16_t TileNum = tile_group->tg_start; TileNum <= tile_group->tg_end; TileNum++) {
		...
		if (lastTile) {
			...
			if (sz < TileSizeBytes) {
				snprintf(err->error, err->size, "Not enough bytes left to read tile size for tile %" PRIu16 ".",
				         TileNum);
				return -1;
			}
...
int obp_parse_metadata(uint8_t *buf, size_t buf_size, OBPMetadata *metadata, OBPError *err) {
	...
	if (metadata->metadata_type == OBP_METADATA_TYPE_HDR_CLL) {
		...
		if (metadata->metadata_scalability.scalability_mode_idc) {
			...
			_obp_br(metadata->metadata_scalability.scalability_structure.temporal_group_description_present_flag, br,
			        1);
			...
			if (metadata->metadata_scalability.scalability_structure.spatial_layer_dimensions_present_flag) {
				for (uint8_t i = 0; i < metadata->metadata_scalability.scalability_structure.spatial_layers_cnt_minus_1;
				     i++) {
			...
			if (metadata->metadata_scalability.scalability_structure.temporal_group_description_present_flag) {
				...
				for (uint8_t i = 0; i < metadata->metadata_scalability.scalability_structure.temporal_group_size; i++) {
					...
					_obp_br(metadata->metadata_scalability.scalability_structure
					...
					for (uint8_t j = 0;
					     j < metadata->metadata_scalability.scalability_structure.temporal_group_ref_cnt[i]; j++) {
						_obp_br(metadata->metadata_scalability.scalability_structure.temporal_group_ref_pic_diff[i][j],
						        br, 8);
					}
				}
			}
...
int obp_parse_frame(uint8_t *buf,
                    size_t buf_size,
...
int obp_parse_frame_header(uint8_t *buf,
                           size_t buf_size,
                           OBPSequenceHeader *seq,
                           OBPState *state,
                           int temporal_id,
	...
	*SeenFrameHeader = 1;
	...
	if (seq->frame_id_numbers_present_flag) {
		...
		for (int i = 0; i < 8; i++) {
			if (fh->current_frame_id > (((uint32_t)1) << diffLen)) {
				if (state->RefFrameId[i] > fh->current_frame_id ||
				    state->RefFrameId[i] < (fh->current_frame_id - (1 << diffLen))) {
					state->RefValid[i] = 0;
				}
			} else {
				if (state->RefFrameId[i] > fh->current_frame_id &&
				    state->RefFrameId[i] < ((1 << idLen) + fh->current_frame_id + (1 << diffLen))) {
					state->RefValid[i] = 0;
				}
			}
		}
		...
		for (int i = 0; i < 7; i++) {
			...
			if (seq->frame_id_numbers_present_flag) {
				...
				if (state->RefFrameId[fh->ref_frame_idx[i]] != expectedFrameId) {
					snprintf(err->error, err->size,
					         "state->RefFrameId[fh->ref_frame_idx[i]] != expectedFrameId (%" PRIu8 " vs %" PRIu8 ")",
	...
	for (int segmentId = 0; segmentId < 8; segmentId++) {
		...
		LosslessArray[segmentId] =
		    (qindex == 0 && DeltaQYDc == 0 && DeltaQUAc == 0 && DeltaQUDc == 0 && DeltaQVAc == 0 && DeltaQVDc == 0);
