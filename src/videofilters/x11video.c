...
static Window createX11Window(X11Video *s) {
	...
	*/
	...
	w = XCreateWindow(s->display, DefaultRootWindow(s->display), 200, 200, s->wsize.width, s->wsize.height, 0,
...
static void x11video_prepare(MSFilter *f) {
	...
	ms_message("x11video_prepare(): Window has size %ix%i, received video is %ix%i", wa.width, wa.height,
	           s->vsize.width, s->vsize.height);
	...
	if (XvQueryAdaptors(s->display, DefaultRootWindow(s->display), &nadaptors, &xai) != 0) {
	...
	for (n = 0; n < nadaptors && port == (XvPortID)-1; ++n) {
		...
		ms_message("Found output adaptor; name=%s num_ports=%i, with %i formats:", ai->name, (int)ai->num_ports,
		           (int)ai->num_formats);
		imgfmt = XvListImageFormats(s->display, ai->base_id, &nimgfmt);
		for (i = 0; i < nimgfmt; ++i) {
			char fcc[5] = {0};
			memcpy(fcc, &imgfmt[i].id, 4);
			ms_message("type=%s/%s id=%s", imgfmt[i].type == XvYUV ? "YUV" : "RGB",
	...
	if (shminfo->shmid == -1) {
		ms_error("Could not allocate %i bytes of shared memory: %s", s->xv_image->data_size, strerror(errno));
		x11video_unprepare(f);
		return;
	}
...
static void x11video_process(MSFilter *f) {
		...
		if (ms_yuv_buf_init_from_mblk(&src, inm) == 0) {
			...
			if (!ms_video_size_equal(newsize, obj->vsize)) {
				...
				if (obj->autofit) {
					...
					ms_message("autofit: new window size should be %ix%i", new_window_size.width,
	...
	if (update) {
		...
		XvShmPutImage(obj->display, obj->port, obj->window_id, obj->gc, obj->xv_image, 0, 0, obj->fbuf.w, obj->fbuf.h,
...
static MSFilterMethod methods[] = {{MS_FILTER_SET_VIDEO_SIZE, x11video_set_vsize},
...
MSFilterDesc ms_x11video_desc = {.id = MS_X11VIDEO_ID,
                                 .name = "MSX11Video",
