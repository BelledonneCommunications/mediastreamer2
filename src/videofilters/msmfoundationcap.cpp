...
public:
	...
	Format getMediaConfiguration(GUID *videoFormat,
	                             UINT32 *width,
	                             UINT32 *height,
...
MSMFoundationCap::VideoFormat::VideoFormat() {
	mWidth = 0;
	mHeight = 0;
	mFps = 0;
	mVideoFormat = MFVideoFormat_Base;
}

MSMFoundationCap::VideoFormat::VideoFormat(UINT32 pWidth, UINT32 pHeight, int pFps, GUID pVideoFormat) {
	mWidth = pWidth;
	mHeight = pHeight;
	mFps = pFps;
	mVideoFormat = pVideoFormat;
}
bool MSMFoundationCap::VideoFormat::isEqual(const VideoFormat &format) const {
	return mWidth == format.mWidth && mHeight == format.mHeight && mFps == format.mFps &&
	       mVideoFormat == format.mVideoFormat;
}
bool MSMFoundationCap::VideoFormat::isBlacklisted(const std::list<VideoFormat> &blacklist) const {
	auto it =
	    std::find_if(blacklist.begin(), blacklist.end(), [&](const VideoFormat &format) { return isEqual(format); });
...
void MSMFoundationCap::feed(MSFilter *filter) {
	if (mFmtChanged) { // Keep this if we want to manage camera behaviors (format changing not coming from Linphone)
		ms_message("[MSMFoundationCap] Camera has changed its own output format. Sending event (%dx%d)", mWidth,
		           mHeight);
	...
	if (!mNewFormatTakenAccount || !mNewFormatValidated)
...
void MSMFoundationCap::setVideoFormat(const GUID &videoFormat) {
	if (videoFormat == MFVideoFormat_Base || !isSupportedFormat(videoFormat)) { // Default format
		...
		if (mVideoFormat == MFVideoFormat_NV12) mPixelFormat = MS_YUV420P;
		else if (mVideoFormat == MFVideoFormat_MJPG) mPixelFormat = MS_MJPEG;
		else if (mVideoFormat == MFVideoFormat_YUY2) mPixelFormat = MS_YUY2;
	}
}

HRESULT
MSMFoundationCap::restartWithNewConfiguration(GUID videoFormat, UINT32 frameWidth, UINT32 frameHeight, float pFps) {
	return S_OK;
}
...
void MSMFoundationCap::processFrame(byte *inputBytes, DWORD inputCapacity, int inputStride) {
	...
	if (mNewFormatValidated && mNewFormatTakenAccount) {
		if (mVideoFormat == MFVideoFormat_NV12) {       // Process raw data from NV12
			if (inputCapacity >= mHeight * inputStride) // Ensure to get enough data in frame
				mFrameData = copy_ycbcrbiplanar_to_true_yuv_with_rotation(
				    mAllocator, inputBytes, inputBytes + mHeight * abs(inputStride), mOrientation, mWidth, mHeight,
				    inputStride, inputStride, TRUE);
		} else if (mVideoFormat == MFVideoFormat_MJPG ||
		           mVideoFormat == MFVideoFormat_YUY2) { // Process raw data from MJPEG/YUY2
...
static int ms_mfoundation_get_pix_fmt(MSFilter *filter, void *arg) {
	MSMFoundationCap *mf = (MSMFoundationCap *)filter->data;
	if (mf->mPixelFormat == MS_RGB24) *((MSPixFmt *)arg) = (MSPixFmt)MS_RGB24_REV;
	else *((MSPixFmt *)arg) = (MSPixFmt)mf->mPixelFormat;
	return 0;
}
...
static int ms_mfoundation_get_vsize(MSFilter *filter, void *arg) {
	...
	vs->width = mf->mWidth;
	if (!mf->mFmtChanged)
		mf->mNewFormatTakenAccount =
		    TRUE; // We get a size when thare are no format changes: we suppose that the new format is taken account.
	return 0;
}
...
MSFilterDesc ms_mfoundation_read_desc = {MS_FILTER_PLUGIN_ID,
                                         "MSMFoundationCap",
...
MSWebCamDesc ms_mfoundationcap_desc = {"MSMFoundationCamera",           ms_mfoundationcap_detect, NULL,
                                       ms_mfoundationcap_create_reader, ms_mfoundationcap_uinit,  NULL};
#ifdef __cplusplus
} // extern "C"
#endif
...
#define IF_EQUAL_GUID_RETURN(guid, sub, type)                                                                          \
	if (type == std::string(MSMFoundationCap::pixFmtToString(guid)) || type == sub) return guid
#endif
...
public:
	...
	void reader_FrameArrived(Windows::Media::Capture::Frames::MediaFrameReader ^ reader,
		...
		if (gCurrentCap) gCurrentCap->reader_FrameArrived(reader, args);
	}
};
			...
			if (format->VideoFormat) {
				...
				int framerate =
				    (int)((format->FrameRate
				               ? static_cast<float>(format->FrameRate->Numerator / format->FrameRate->Denominator)
				               : 0.0f) *
				          100.0f);
...
task<bool> MSMFoundationUwpImpl::tryInitializeCaptureAsync() {
	...
	settings->MemoryPreference =
	    MediaCaptureMemoryPreference::Cpu; // Set to CPU to ensure frames always contain CPU SoftwareBitmap
	                                       // images,instead of preferring GPU D3DSurface images.
	settings->StreamingCaptureMode =
	    StreamingCaptureMode::Video; // Capture only video. Audio device will not be initialized.
	try {
		...
		if (e->HResult == 0xc00d3704 && mCameraSharingMode == MediaCaptureSharingMode::ExclusiveControl) {
			mCameraSharingMode =
			    MediaCaptureSharingMode::SharedReadOnly; // This media capture has shared control of the source.
			disposeMediaCapture();
		...
		if (mSource != nullptr) {
			...
			try {
				MediaFrameReader ^ reader =
				    create_task(mMediaCapture->CreateFrameReaderAsync(mSource, mSource->CurrentFormat->Subtype)).get();
				mReader = reader;
			...
			LeaveCriticalSection(&mCriticalSection);
			if (needToCreateReader) taskResult = createReaderAsync();
		} else {
...
void MSMFoundationUwpImpl::processFrame(Windows::Media::Capture::Frames::MediaFrameReference ^ frame) {
	...
	int inputStride = input->GetPlaneDescription(0)
	                      .Stride; // Get stride values to calculate buffer position for a given pixel x and y position.
	IMemoryBufferReference ^ inputReference = input->CreateReference();
	byte *inputBytes;
	UINT32 inputCapacity;
	if (inputBitmap->PixelWidth == mWidth && inputBitmap->PixelHeight == mHeight) {
...
bool_t MSMFoundationUwpImpl::setInternalFormat(GUID videoFormat, UINT32 frameWidth, UINT32 frameHeight, float pFps) {
	bool_t formatChanged = mVideoFormat != videoFormat;
...
MSMFoundationUwpImpl::setMediaConfiguration(GUID videoFormat, UINT32 frameWidth, UINT32 frameHeight, float pFps) {
	...
	if (mSource) {
		...
		if (!isSupportedFormat(videoFormat)) {
			ms_error("[MSMFoundationCapUwp] The Video format is not supported by the filter : %s. Trying to force to "
			         "MFVideoFormat_NV12",
			         pixFmtToString(videoFormat));
...
public:
	...
	void setMediaTypes(IMFSourceReader *source) {
		...
		for (DWORD i = 0; source->GetNativeMediaType((DWORD)MF_SOURCE_READER_FIRST_VIDEO_STREAM, i, &mediaType) !=
		                  MF_E_NO_MORE_TYPES;
		     ++i) {
			++mediaCount;
			if (mediaType) {
				...
				if (SUCCEEDED(hr) && MSMFoundationCap::isSupportedFormat(videoType)) { // Get frame size from media type
					...
					else traces.push_back("Cannot get Format from MF_MT_SUBTYPE: [" + std::to_string(hr) + "]");
					hr = -1;
				}
				if (SUCCEEDED(hr)) {
					traces.push_back("Attributes could be retrieved: " + std::to_string(width) + "/" +
				...
				if (SUCCEEDED(hr)) {
					mSortedList[width][height][(int)((static_cast<float>(fpsNumerator) / fpsDenominator) * 100.0f)]
					           [videoType] = mediaType;
		...
		// Debug mode
		if (mSortedList.size() == 0) {
			ms_error("There are no available configurations. Media:%d UsableMedia:%d. Traces:", mediaCount,
			         acceptableMediaCount);
...
void MSMFoundationDesktopImpl::start() {
	...
	while (!found && currentDeviceIndex < devices.mDevicesCount) {
		...
		UINT32 cchName;
		hr = devices.mDevices[currentDeviceIndex]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME, &nameString,
		                                                              &cchName);
		if (SUCCEEDED(hr)) {
			...
			if (WideCharToMultiByte(currentCodePage, 0, nameString, (int)inputlen, &strConversion[0], (int)sizeNeeded,
			                        NULL, NULL) &&
			    mDeviceName == strConversion) {
				found = TRUE;
	...
	if (found) {
		if (!mSourceReader) {
			...
			ms_average_fps_init(&mAvgFps, "[MSMFoundationCapDesk] fps=%f");
		}
	} else {
		ms_error("[MSMFoundationCapDesk] Device cannot be activated because friendly name has not been found : '%s' in "
		...
		for (UINT32 i = 0; i < devices.mDevicesCount; ++i) {
			...
			if (SUCCEEDED(hr)) {
				...
				UINT currentCodePage = GetACP();
				int sizeNeeded =
				    WideCharToMultiByte(currentCodePage, 0, nameString, (int)inputlen, NULL, 0, NULL, NULL);
				std::string strConversion(sizeNeeded, 0);
				if (WideCharToMultiByte(currentCodePage, 0, nameString, (int)inputlen, &strConversion[0],
				                        (int)sizeNeeded, NULL, NULL))
					ms_error("[MSMFoundationCapDesk] %s", strConversion.c_str());
			}
		}
	}
	devices.clean();
...
void MSMFoundationDesktopImpl::deactivate() {
	...
	LeaveCriticalSection(&mCriticalSection);
	ms_message("[MSMFoundationCapDesk] Frames count : %d samples, %d processed", mSampleCount, mProcessCount);
	mSampleCount = mProcessCount = 0;
}
...
STDMETHODIMP MSMFoundationDesktopImpl::QueryInterface(REFIID riid, void **ppvObject) {
	...
	static const QITAB qit[] = {
	    QITABENT(MSMFoundationDesktopImpl, IMFSourceReaderCallback),
	    {0},
	};
...
// From IUnknown
ULONG MSMFoundationDesktopImpl::AddRef() {
	return InterlockedIncrement(&mReferenceCount);
}

// Method from IMFSourceReaderCallback
STDMETHODIMP MSMFoundationDesktopImpl::OnEvent(DWORD, IMFMediaEvent *mediaEvent) {
	return S_OK;
}
// Method from IMFSourceReaderCallback
...
MSMFoundationDesktopImpl::setMediaConfiguration(GUID videoFormat, UINT32 frameWidth, UINT32 frameHeight, float pFps) {
	...
	if (mSourceReader) {
		...
		if (!isSupportedFormat(videoFormat)) {
			ms_error("[MSMFoundationCapDesk] The Video format is not supported by the filter : %s. Trying to force to "
			         "MFVideoFormat_NV12",
			         pixFmtToString(videoFormat));
			videoFormat = MFVideoFormat_NV12;
		}
...
HRESULT MSMFoundationDesktopImpl::restartWithNewConfiguration(GUID videoFormat,
                                                              UINT32 frameWidth,
                                                              UINT32 frameHeight,
                                                              float pFps) {
	ms_message("[MSMFoundationCapDesk] Restarting device with a new configuration : %dx%d : %s, %f fps", frameWidth,
...
HRESULT MSMFoundationDesktopImpl::getStride(IMFMediaType *pType, LONG *stride) {
	...
	hr = pType->GetUINT32(MF_MT_DEFAULT_STRIDE,
	                      (UINT32 *)&tempStride); // Try to get the default stride from the media type.
	if (FAILED(hr)) {
		UINT32 width, height;
		...
		if (SUCCEEDED(hr)) hr = MFGetStrideForBitmapInfoHeader(subtype.Data1, width, &tempStride); //[desktop apps only]
		// set the attribute so it can be read
		if (SUCCEEDED(hr)) (void)pType->SetUINT32(MF_MT_DEFAULT_STRIDE, UINT32(tempStride));
#endif
	}
	if (SUCCEEDED(hr) && stride) *stride = tempStride;
	return hr;
}

// Method from IMFSourceReaderCallback
HRESULT MSMFoundationDesktopImpl::OnReadSample(
    HRESULT status, DWORD streamIndex, DWORD streamFlags, LONGLONG timeStamp, IMFSample *sample) {
	HRESULT hr = status;
...
static void ms_mfoundationcap_detect(MSWebCamManager *manager) {
	...
	for (UINT32 i = 0; i < devices.mDevicesCount; ++i) { // Get the human-friendly name of the device
		...
		UINT32 cchName;
		HRESULT hr =
		    devices.mDevices[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME, &nameString, &cchName);
		if (SUCCEEDED(hr)) {
			// We don't need null-terminated character because we are using std::string for convertion.
			size_t inputlen = wcslen(nameString);
			UINT currentCodePage = GetACP();
			...
			std::string strConversion(sizeNeeded, 0);
			char *nameStr = NULL;
			if (WideCharToMultiByte(currentCodePage, 0, nameString, (int)inputlen, &strConversion[0], sizeNeeded, NULL,
			                        NULL)) {
			...
			if (!nameStr) {
				ms_error("[MSMFoundationCapDesk] Cannot convert webcam name to multi-byte string.");
				ms_free(nameStr);
			} else {
				MSWebCam *cam = ms_web_cam_new(&ms_mfoundationcap_desc);
				cam->name = nameStr;
