...
static mblk_t *get_as_yuvmsg(MSFilter *f, DecData *s, AVFrame *orig) {
	...
	if (s->vsize.width != ctx->width || s->vsize.height != ctx->height) {
		...
		s->sws_ctx = sws_getContext(ctx->width, ctx->height, ctx->pix_fmt, ctx->width, ctx->height, AV_PIX_FMT_YUV420P,
		                            SWS_FAST_BILINEAR, NULL, NULL, NULL);
		ms_filter_notify_no_arg(f, MS_FILTER_OUTPUT_FMT_CHANGED);
	}
	yuv_msg = ms_yuv_buf_allocator_get(s->buf_allocator, &pic, ctx->width, ctx->height);
#if LIBSWSCALE_VERSION_INT >= AV_VERSION_INT(0, 9, 0)
	if (sws_scale(s->sws_ctx, (const uint8_t *const *)orig->data, orig->linesize, 0, ctx->height, pic.planes,
	              pic.strides) < 0) {
#else
	if (sws_scale(s->sws_ctx, (uint8_t **)orig->data, orig->linesize, 0, ctx->height, pic.planes, pic.strides) < 0) {
#endif
	...
	*new_sps_pps = FALSE;
	...
	while ((im = ms_queue_get(naluq)) != NULL) {
		...
		if (src[0] == 0 && src[1] == 0 && src[2] == 0 && src[3] == 1) {
			...
			nalu_type = (*src) & ((1 << 5) - 1);
			if (nalu_type == 7) *new_sps_pps = (check_sps_change(d, im) || *new_sps_pps);
			if (nalu_type == 8) *new_sps_pps = (check_pps_change(d, im) || *new_sps_pps);
...
static int dec_get_outfmt(MSFilter *f, void *data) {
	DecData *s = (DecData *)f->data;
	((MSPinFormat *)data)->fmt = ms_factory_get_video_format(
	    f->factory, "YUV420P", ms_video_size_make(s->vsize.width, s->vsize.height), 0, NULL);
	return 0;
}
...
static MSFilterMethod h264_dec_methods[] = {{MS_FILTER_ADD_FMTP, dec_add_fmtp},
...
MSFilterDesc ms_h264_dec_desc = {.id = MS_H264_DEC_ID,
                                 .name = "MSH264Dec",
...
MSFilterDesc ms_h264_dec_desc = {MS_H264_DEC_ID,
                                 "MSH264Dec",
