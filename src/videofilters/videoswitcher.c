...
static void
switcher_transfer(MSFilter *f, MSQueue *input, MSQueue *output, OutputContext *output_context, mblk_t *start) {
	mblk_t *m;
	...
	for (i = 0; i < f->desc->noutputs; ++i) {
		...
		if (q) {
			...
			if (output_context->current_source != -1 && f->inputs[output_context->current_source] == NULL) {
				ms_warning("%s: current source %i disapeared, choosing another one to switch to.", f->desc->name,
				           output_context->current_source);
			...
			if (output_context->current_source != -1) {
				...
				if (input_context->state == RUNNING) {
					...
					// ms_message("%s: transfered packets from pin %i to pin %i", f->desc->name,
					// output_context->current_source, i);
				}
			}
		}
	}
...
static MSFilterMethod methods[] = {{MS_VIDEO_SWITCHER_SET_FOCUS, switcher_set_focus},
