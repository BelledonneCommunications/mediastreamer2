...
bool_t is_vp8_key_frame(mblk_t *m) {
	...
	if (!p) {
		ms_warning("video-conference-util: invalid vp8 payload descriptor.");
		return FALSE;
	}
	return !(p[0] & 1);
}
...
void filter_channel_update_input(MSVideoConferenceFilterState *s, InputContext *input_context, int pin, MSQueue *q){
	...
	for(m = ms_queue_peek_first(q); !ms_queue_end(q, m); m = ms_queue_peek_next(q,m)){
		...
		if (!input_context->seq_set){
			...
			ms_warning("video-conference-util: Sequence discontinuity detected on pin %i, key-frame requested", pin);
			input_context->state = STOPPED;
			input_context->key_frame_requested = TRUE;
		}
		if (input_context->key_frame_requested){
			if (!input_context->seq_set || input_context->cur_ts != new_ts){
				...
				if (s->mIsKeyFrame(m)){
					ms_message("video-conference-util: key frame detected on pin %i", pin);
...
int filter_set_fmt(MSFilter *f, void *data) {
	...
	MSVideoConferenceFilterState *s=(MSVideoConferenceFilterState *)f->data;
	if (fmt){
		if (strcasecmp(fmt->encoding,"VP8")==0){
			s->mIsKeyFrame=is_vp8_key_frame;
		}else if (strcasecmp(fmt->encoding,"H264")==0){
			s->mIsKeyFrame=is_h264_key_frame;
		}else{
