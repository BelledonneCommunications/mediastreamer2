 ...
 * Dimensions are auto-managed and are retrieved from EGL. If they cannot be retrieved for any reason, the input
 ...
 */
MS2_PUBLIC void ogl_display_set_target_context(struct opengles_display *gldisp,
...
/**
 * Render display. It will update viewport if sizes can be retrieved from EGL Surface. If not, the last size will be
 * used.
 *
...
/**
 * @params contains the zoom parameters: [0] = zoom_factor, [1][2] = zoom center x/y (between [0,1], relative coords to
 * the gl surface. 0.5/0.5 = center)
 */
...
JNIEXPORT void JNICALL Java_org_linphone_mediastream_video_display_OpenGLESDisplay_render(JNIEnv *env,
                                                                                          jobject obj,
                                                                                          jlong ptr);
#endif
