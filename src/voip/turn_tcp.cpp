...
int PacketReader::processContinuationPacket(std::unique_ptr<Packet> packet) {
	size_t to_read = std::min(packet->length(), mRemainingBytes);
...
SslContext::SslContext(ortp_socket_t socket, std::string rootCertificatePath, std::string cn) : mSocket(socket) {
	...
	bctbx_ssl_config_set_rng(mConfig, random_generator, NULL);
...
int TurnSocket::connect() {
	...
	int optVal = 1;
	if (setsockopt(mSocket, IPPROTO_TCP, TCP_NODELAY, (const void *)&optVal, sizeof(optVal)) != 0) {
	...
	if (mClient->mUseSsl) {
		mSsl = std::make_unique<SslContext>(mSocket, mClient->mRootCertificatePath, mClient->mTurnServerCn);
	...
	optVal = 1200 * 8;
	error = setsockopt(mSocket, SOL_SOCKET, SO_SNDBUF, (const void *)&optVal, sizeof(optVal));
	if (error != 0) {
	...
	tv.tv_sec = 1;
	tv.tv_usec = 0;
	error = setsockopt(mSocket, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv));
	if (error != 0) {
