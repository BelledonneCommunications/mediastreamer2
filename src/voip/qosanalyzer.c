...
MSQosAnalyzerAlgorithm ms_qos_analyzer_algorithm_from_string(const char *alg) {
	if (alg == NULL || strcasecmp(alg, "Simple") == 0) return MSQosAnalyzerAlgorithmSimple;
	else if (strcasecmp(alg, "Stateful") == 0) return MSQosAnalyzerAlgorithmStateful;
...
static bool_t simple_analyzer_process_rtcp(MSQosAnalyzer *objbase, mblk_t *rtcp) {
	...
	if (rb && report_block_get_ssrc(rb) == rtp_session_get_send_ssrc(obj->session)) {
		...
		if (obj->clockrate == 0) {
			PayloadType *pt = rtp_profile_get_payload(rtp_session_get_send_profile(obj->session),
...
static void simple_analyzer_suggest_action(MSQosAnalyzer *objbase, MSRateControlAction *action) {
	...
	if (objbase->on_action_suggested != NULL) {
		...
		data[1] = ms_strdup_printf("%d %d %d %d", (int)cur->lost_percentage, (simple_rt_prop_increased(obj) == TRUE),
		                           (int)cur->int_jitter, (int)(1000 * cur->rt_prop));
		data[2] = ms_strdup("action_type action_value");
		data[3] = ms_strdup_printf("%s %d", ms_rate_control_action_type_name(action->type), action->value);
...
static MSQosAnalyzerDesc simple_analyzer_desc = {simple_analyzer_process_rtcp, simple_analyzer_suggest_action,
                                                 simple_analyzer_has_improved, NULL, NULL};
...
static float stateful_qos_analyzer_upload_bandwidth(MSStatefulQosAnalyzer *obj, uint32_t seq_num) {
	...
	for (latest_bw = 0; latest_bw < BW_HISTORY; ++latest_bw) {
		ms_debug("MSStatefulQosAnalyzer[%p]:\t%u\t-->\t%f", obj, obj->upload_bandwidth[latest_bw].seq_number,
	...
	if (obj->upload_bandwidth[(obj->upload_bandwidth_cur + 1) % BW_HISTORY].seq_number > seq_num) {
		ms_warning("MSStatefulQosAnalyzer[%p]: saved to much points - seq_number lower "
		           "than oldest measure! Increase BW_HISTORY or reduce ptime!",
		           obj);
	} else {
		int count = 0;
		...
		if (count == 0) {
			ms_error("MSStatefulQosAnalyzer[%p]: no measures available to compute bandwidth for ext_seq=%u", obj,
			         seq_num);
	...
	ms_message("MSStatefulQosAnalyzer[%p]: bw_curent=%f vs bw_per_avg=%f vs bw_per_seqnum=%f", obj,
	           rtp_session_get_send_bandwidth(obj->session) / 1000.0, bw_per_avg, bw_per_seqnum);
...
static bool_t stateful_analyzer_process_rtcp(MSQosAnalyzer *objbase, mblk_t *rtcp) {
	...
	if (rb && report_block_get_ssrc(rb) == rtp_session_get_send_ssrc(obj->session)) {
		if (ortp_loss_rate_estimator_process_report_block(objbase->lre, obj->session, rb)) {
			...
			obj->rtcpstatspoint =
			    bctbx_list_insert_sorted(obj->rtcpstatspoint, obj->latest, (bctbx_compare_func)sort_by_bandwidth);
			...
			ms_message("MSStatefulQosAnalyzer[%p]: one more %d: %f %f", obj, obj->curindex - 1, obj->latest->bandwidth,
			           obj->latest->loss_percent);
			...
			if (bctbx_list_size(obj->rtcpstatspoint) > ESTIM_HISTORY) {
				...
				obj->rtcpstatspoint =
				    bctbx_list_remove_custom(obj->rtcpstatspoint, (bctbx_compare_func)earlier_than, &clear_time);
...
static double compute_available_bw(MSStatefulQosAnalyzer *obj) {
	...
	for (it = obj->rtcpstatspoint; it != NULL; it = it->next) {
		...
		(void)point;
		ms_message("MSStatefulQosAnalyzer[%p]:\t\tsorted values %d: %f %f", obj,
		           bctbx_list_position(obj->rtcpstatspoint, it), point->bandwidth, point->loss_percent);
	}
#endif
	...
	if (size == 1) {
		...
		while (current != NULL && ((rtcpstatspoint_t *)current->data)->loss_percent < 3 + constant_network_loss) {
			ms_message("MSStatefulQosAnalyzer[%p]:\t%d is stable", obj,
			...
			for (it = last; it != current; it = it->prev) {
				if (((rtcpstatspoint_t *)it->data)->loss_percent <=
				    3 + ((rtcpstatspoint_t *)current->data)->loss_percent) {
					ms_message("MSStatefulQosAnalyzer[%p]:\t%d is less than %d", obj,
					           bctbx_list_position(obj->rtcpstatspoint, it),
					           bctbx_list_position(obj->rtcpstatspoint, current));
					current = it;
					break;
				}
		...
		ms_message("MSStatefulQosAnalyzer[%p]: [0->%d] last stable is %d(%f;%f)", obj,
		...
		if (current != NULL) {
			ms_message("MSStatefulQosAnalyzer[%p]: , first unstable is %d(%f;%f)", obj,
			           bctbx_list_position(obj->rtcpstatspoint, current),
	...
	*/
	if (obj->curindex == 1) {
		...
		greatest_pt = bctbx_list_size(obj->rtcpstatspoint)
		                  ? (rtcpstatspoint_t *)bctbx_list_nth_data(obj->rtcpstatspoint,
		                                                            (int)bctbx_list_size(obj->rtcpstatspoint) - 1)
		                  : NULL;
	...
	ms_message("MSStatefulQosAnalyzer[%p]: %s of value %d", obj, ms_rate_control_action_type_name(action->type),
	           action->value);
	...
	if (objbase->on_action_suggested != NULL) {
		...
		data[1] =
		    ms_strdup_printf("%d %d %d", obj->latest ? (int)obj->latest->loss_percent : 0,
		                     obj->latest ? (int)obj->latest->rtt : 0, obj->latest ? (int)obj->latest->bandwidth : 0);
		data[2] = ms_strdup("action_type action_value est_bw");
		data[3] = ms_strdup_printf("%s %d %d", ms_rate_control_action_type_name(action->type), action->value, (int)bw);
...
static void stateful_analyzer_update(MSQosAnalyzer *objbase) {
	...
	if (last_measure != ms_time(0)) {
		...
		obj->upload_bandwidth[obj->upload_bandwidth_cur].up_bandwidth =
		    rtp_session_get_send_bandwidth(obj->session) / 1000.0f;
	...
	if (obj->burst_duration_ms > 0) {
		switch (obj->burst_state) {
			...
			case MSStatefulQosAnalyzerBurstInProgress: {
				...
				elapsed = ((now.tv_sec - obj->start_time.tv_sec) * 1000.0f) +
				          ((now.tv_usec - obj->start_time.tv_usec) / 1000.0f);
...
static MSQosAnalyzerDesc stateful_analyzer_desc = {
    stateful_analyzer_process_rtcp, stateful_analyzer_suggest_action, stateful_analyzer_has_improved,
    stateful_analyzer_update,       stateful_analyzer_uninit,
};
