...
static void apply_bitrate_limit(VideoStream *stream, PayloadType *pt) {
	...
	if (stream->ms.target_bitrate > 0){
		...
		if (pt->normal_bitrate <= 0) {
			...
			ms_message("Max target bitrate not set for stream [%p], but using payload type's bitrate [%i]",stream,pt->normal_bitrate);
		...
		if (stream->source_performs_encoding == TRUE) {
			...
			if (stream->ms.encoder==NULL){
				...
				if (stream->fallback_to_dummy_codec == TRUE) {
					ms_error("videostream.c: No encoder available for payload %i:%s. Try to create a dummy one",payload,pt->mime_type);
	...
	if (media_stream_get_direction(&stream->ms) == MediaStreamSendRecv || media_stream_get_direction(&stream->ms) == MediaStreamRecvOnly) {
		...
		if (!rtp_output) {
			...
			if (stream->ms.decoder==NULL){
				...
				if (stream->fallback_to_dummy_codec == TRUE) {
					ms_error("videostream.c: No decoder available for payload %i:%s. Try to create a dummy one",payload,pt->mime_type);
