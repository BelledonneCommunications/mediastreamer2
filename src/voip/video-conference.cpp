...
void VideoConferenceGeneric::addVideoPlaceholderMember() {
...
void VideoConferenceGeneric::addMember(MSVideoEndpoint *ep) {
...
void VideoConferenceGeneric::removeMember(MSVideoEndpoint *ep) {
	video_stream_set_encoder_control_callback(ep->st, NULL, NULL);
	ms_ticker_detach(mTicker,mMixer);
	unConfigureOutput(ep->pin);
...
void VideoConferenceAllToAll::configureOutput(int pin) {
	if (mMembers == NULL) {
		ms_filter_call_method(mMixer, MS_VIDEO_ROUTER_CONFIGURE_OUTPUT, &pin);
	}
}
