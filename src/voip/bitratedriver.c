...
int ms_bitrate_driver_execute_action(MSBitrateDriver *obj, const MSRateControlAction *action) {
	if (obj->desc->execute_action) return obj->desc->execute_action(obj, action);
...
static int apply_ptime(MSAudioBitrateDriver *obj, int target_ptime) {
	...
	if (target_ptime < min_ptime || target_ptime > max_ptime) {
		ms_error("MSAudioBitrateDriver [%p]: cannot apply ptime value [%i] on [%p] because out of range [%i..%i]", obj,
		         target_ptime, obj, min_ptime, max_ptime);
		return -1;
	}
	...
	if (result == 0) {
		...
		ms_message("MSAudioBitrateDriver [%p]: cannot move ptime from [%i ms] to [%i ms]", obj, obj->cur_ptime,
		           target_ptime);
	}
	return result;
}
...
static int audio_bitrate_driver_execute_action(MSBitrateDriver *objbase, const MSRateControlAction *action) {
	...
	ms_message("MSAudioBitrateDriver: executing action of type %s, value=%i",
	...
	if (obj->nom_bitrate == 0) {
		...
		if (obj->nom_bitrate == 0) {
			ms_warning("MSAudioBitrateDriver: Not doing bitrate control on audio encoder, it does not seem to support "
			           "that. Controlling ptime only.");
			obj->nom_bitrate = -1;
		} else obj->cur_bitrate = obj->nom_bitrate;
	}
	if (obj->cur_ptime == 0 ||
	    ms_filter_has_method(obj->encoder, MS_AUDIO_ENCODER_GET_PTIME)) { /*always sync current ptime if possible*/
		ms_filter_call_method(obj->encoder, MS_AUDIO_ENCODER_GET_PTIME, &obj->cur_ptime);
		if (obj->cur_ptime == 0) {
			ms_warning("MSAudioBitrateDriver: encoder %s does not implement MS_AUDIO_ENCODER_GET_PTIME. Consider to "
			           "implement this method for better accuracy of rate control.",
			           obj->encoder->desc->name);
...
static void audio_bitrate_driver_uninit(BCTBX_UNUSED(MSBitrateDriver *objbase)) {
	// MSAudioBitrateDriver *obj=(MSBitrateDriver*)objbase;
}

static MSBitrateDriverDesc audio_bitrate_driver = {audio_bitrate_driver_execute_action, audio_bitrate_driver_uninit};
...
static int av_driver_execute_action(MSBitrateDriver *objbase, const MSRateControlAction *action) {
	...
	if (obj->nom_bitrate == 0) {
		...
		if (obj->nom_bitrate == 0) {
			ms_warning("MSAVBitrateDriver: Not doing adaptive rate control on video encoder, it does not seem to "
			           "support that.");
			return -1;
		}
	}
...
static MSBitrateDriverDesc av_bitrate_driver = {av_driver_execute_action, av_bitrate_driver_uninit};
...
MSBitrateDriver *ms_av_bitrate_driver_new(RtpSession *asession, MSFilter *aenc, RtpSession *vsession, MSFilter *venc) {
	...
	obj->audio_driver = ((asession != NULL) && (aenc != NULL))
	                        ? ms_bitrate_driver_ref(ms_audio_bitrate_driver_new(asession, aenc))
	                        : NULL;
...
static int bandwidth_change_video_bitrate(MSBandwidthBitrateDriver *obj, const MSRateControlAction *action) {
	...
	if ((decrease && newbr < bound) || (!decrease && newbr > bound)) {
		if (obj->cur_bitrate == bound) {
			ms_message("MSBandwidthBitrateDriver: bitrate already reached %s limit %d b/s.", (decrease) ? "min" : "max",
			           bound);
			return -1;
		}
		newbr = bound;
...
static int bandwidth_driver_execute_action(MSBitrateDriver *objbase, const MSRateControlAction *action) {
	...
	if (obj->nom_bitrate == 0 && obj->venc) {
		ms_filter_call_method(obj->venc, MS_FILTER_GET_BITRATE, &obj->nom_bitrate);
		if (obj->nom_bitrate == 0) {
			ms_warning("MSBandwidthBitrateDriver: Not doing adaptive rate control on video encoder, it does not seem "
			           "to support that.");
			return -1;
		}
	}
	...
	ms_message("MSBandwidthBitrateDriver: Action %s %s", ms_rate_control_action_type_name(action->type),
