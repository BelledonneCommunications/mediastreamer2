...
static int turnPoll(ortp_socket_t socket, int seconds, bool write) {
	fd_set fds;
	struct timeval tv;
	...
	if (write) {
		return select(mFd + 1, NULL, &fds, NULL, &tv);
	} else {
		return select(mFd + 1, &fds, NULL, NULL, &tv);
	}
}
...
class SslContext {
  ...
  public:
	SslContext(ortp_socket_t socket, std::string rootCertificatePath, std::string cn);
	~SslContext();
