	...
	switch(event_id) {
		case MS_RTP_RECV_MIXER_TO_CLIENT_AUDIO_LEVEL_RECEIVED:
			...
			for(i = 0; i < RTP_MAX_MIXER_TO_CLIENT_AUDIO_LEVEL && mtc_volumes[i].csrc != 0; i++) {
				audio_stream_volumes_insert(as->participants_volumes, mtc_volumes[i].csrc, ms_volume_dbov_to_dbm0(mtc_volumes[i].dbov));
			}
			break;
...
AudioStream *audio_stream_new_with_sessions(MSFactory *factory, const MSMediaStreamSessions *sessions){
	...
	stream->mixer_to_client_extension_id = RTP_EXTENSION_MIXER_TO_CLIENT_AUDIO_LEVEL;
...
int audio_stream_get_participant_volume(const AudioStream *stream, uint32_t participant_ssrc) {
	int volume = audio_stream_volumes_find(stream->participants_volumes, participant_ssrc);
	if (volume != AUDIOSTREAMVOLUMES_NOT_FOUND) {
		return volume;
	}
