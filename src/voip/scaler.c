...
void ms_line_scale_8(
    const uint32_t *grid, const int16_t *const src[], int16_t *dst[], int dst_width, const int16_t *filter);
...
#if !MS_HAS_ARM_NEON || TARGET_OS_IPHONE

static inline void yuv2rgb_4x2(const uint8_t *y1,
                               const uint8_t *y2,
                               const uint8_t *u,
                               const uint8_t *v,
                               int16_t *r1,
...
void img_ycrcb420p_to_bgra(uint8_t *src[], unsigned short w, unsigned short h, uint32_t dest[]) {
	...
	for (row = 0; row < h; row += 2) {
		...
		for (col_y = 0; col_y < w; col_y += 4) {
			...
			yuv2rgb_4x2(src[0] + offset_y + col_y, src[0] + offset_y + w + col_y, (src[1] + offset_cbcr + col_crcb),
			            (src[2] + offset_cbcr + col_crcb), r1, g1, b1, r2, g2, b2);
			for (i = 0; i < 4; i++) {
...
static inline void
line_yuv2rgb_2(const uint8_t *src_lines[], int src_strides[], int16_t *dst_lines[], int src_w, int dst_stride) {
	int i;
	...
	for (i = 0; i < src_w; i += 4) {
		yuv2rgb_4x2(y1, y2, u, v, dst_lines[0] + i, dst_lines[1] + i, dst_lines[2] + i, line2[0] + i, line2[1] + i,
		            line2[2] + i);
		y1 += 4;
		y2 += 4;
		u += 2;
...
#if MS_HAS_ARM_NEON_32 && !TARGET_OS_IPHONE
...
static void
img_yuv2rgb565_scale(AndroidScalerCtx *ctx, uint8_t *src[], int src_strides[], uint8_t *dst[], int dst_strides[]) {
	int i, pos, y = 0;
	int16_t *p_src[3];
	uint8_t *p_dst = dst[0];
	int offset;
...
static MSScalerContext *android_create_scaler_context(
    int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h, MSPixFmt dst_fmt, BCTBX_UNUSED(int flags)) {
...
static int
android_scaler_process(MSScalerContext *ctx, uint8_t *src[], int src_strides[], uint8_t *dst[], int dst_strides[]) {
	img_yuv2rgb565_scale((AndroidScalerCtx *)ctx, src, src_strides, dst, dst_strides);
	return 0;
}

MSScalerDesc ms_android_scaler = {android_create_scaler_context, android_scaler_process, android_scaler_context_free};
