...
enum state_t { Init, Probing, Stable, ProbingUp };
...
MSBitrateController *
ms_audio_bitrate_controller_new(RtpSession *session, MSFilter *encoder, BCTBX_UNUSED(unsigned int flags)) {
	return ms_bitrate_controller_new(ms_simple_qos_analyzer_new(session),
...
ms_av_bitrate_controller_new(RtpSession *asession, MSFilter *aenc, RtpSession *vsession, MSFilter *venc) {
	return ms_bitrate_controller_new(ms_simple_qos_analyzer_new(vsession),
...
ms_bandwidth_bitrate_controller_new(RtpSession *asession, MSFilter *aenc, RtpSession *vsession, MSFilter *venc) {
	return ms_bitrate_controller_new(ms_stateful_qos_analyzer_new(vsession ? vsession : asession),
