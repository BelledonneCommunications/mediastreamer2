...
static int ice_send_message_to_socket(RtpTransport * rtptp, char* buf, size_t len, const struct sockaddr *from, BCTBX_UNUSED(socklen_t fromlen), const struct sockaddr *to, socklen_t tolen) {
...
static IceStunServerRequestTransaction * ice_send_stun_request(RtpTransport *rtptp, const struct sockaddr *source, socklen_t sourcelen, const struct sockaddr *server, BCTBX_UNUSED(socklen_t addrlen), MSStunMessage *msg, const char *request_type)
{
	IceStunServerRequestTransaction *transaction = NULL;
	char *buf = NULL;
	size_t len;
...
static void ice_generate_arbitrary_foundation(char *foundation, int len, bctbx_list_t *list)
{
	...
	do {
		r = (((uint64_t)bctbx_random()) << 32) | (((uint64_t)bctbx_random()) & 0xffffffff);
...
static IceCandidatePair * ice_trigger_connectivity_check_on_binding_request(IceCheckList *cl, BCTBX_UNUSED(const RtpSession *rtp_session), const OrtpEventData *evt_data, IceCandidate *prflx_candidate, const IceTransportAddress *remote_taddr)
{
...
static int ice_check_received_binding_response_addresses(BCTBX_UNUSED(const RtpSession *rtp_session), const OrtpEventData *evt_data, IceCandidatePair *pair, MSStunAddress *remote_addr) {
	struct sockaddr_storage recv_addr;
	socklen_t recv_addrlen = sizeof(recv_addr);
...
static int ice_check_received_binding_response_attributes(const MSStunMessage *msg, BCTBX_UNUSED(const MSStunAddress *remote_addr), bool_t check_integrity)
{
	if (!ms_stun_message_message_integrity_enabled(msg)) {
...
static IceCandidatePair * ice_construct_valid_pair(IceCheckList *cl, RtpSession *rtp_session, BCTBX_UNUSED(const OrtpEventData *evt_data), IceCandidate *candidate, IceCandidatePair *succeeded_pair)
{
	LocalCandidate_RemoteCandidate candidates;
	IceCandidatePair *pair = NULL;
...
static int ice_find_non_responded_gathering_stun_server_request(const IceStunServerRequest *request, BCTBX_UNUSED(const void *dummy))
{
	...
	if (!ms_compare_stun_addresses(remote_addr, &serv_stun_addr)) {
		...
		if (request != NULL) {
			...
			if ((componentID > 0) && (ice_parse_stun_server_response(msg, &srflx_addr, &relay_addr) >= 0)) {
				...
				if (base_elem != NULL) {
					base_candidate = (IceCandidate *)base_elem->data;
...
static void ice_handle_received_create_permission_success_response(IceCheckList *cl, BCTBX_UNUSED(RtpSession *rtp_session), const OrtpEventData *evt_data, const MSStunMessage *msg, BCTBX_UNUSED(const MSStunAddress *remote_addr)) {
...
static void ice_handle_received_turn_refresh_success_response(IceCheckList *cl, BCTBX_UNUSED(RtpSession *rtp_session), const OrtpEventData *evt_data, const MSStunMessage *msg, BCTBX_UNUSED(const MSStunAddress *remote_addr)) {
	int componentID = ice_get_componentID_from_rtp_session(evt_data);
...
static void ice_handle_received_turn_channel_bind_success_response(IceCheckList *cl, BCTBX_UNUSED(RtpSession *rtp_session), const OrtpEventData *evt_data, const MSStunMessage *msg, BCTBX_UNUSED(const MSStunAddress *remote_addr)) {
	int componentID = ice_get_componentID_from_rtp_session(evt_data);
	UInt96 tr_id = ms_stun_message_get_tr_id(msg);
	IceStunServerRequest *request = ice_check_list_get_stun_server_request(cl, &tr_id);
	if (request != NULL) {
