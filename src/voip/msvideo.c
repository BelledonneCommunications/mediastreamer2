...
int ms_yuv_buf_init_from_mblk_with_size(YuvBuf *buf, mblk_t *m, int w, int h) {
	if (m->b_cont != NULL)
		m = m->b_cont; /*skip potential video header */
	ms_yuv_buf_init(buf, w, h, w, m->b_rptr);
	return 0;
}
...
static void plane_copy(const uint8_t *src_plane, size_t src_row_stride, size_t src_pix_stride, const MSRect *src_roi,
	...
	if (src_row_stride == dst_row_stride && src_pix_stride == 1 && dst_pix_stride == 1 &&
		ms_rect_equal(src_roi, dst_roi)) {
...
void ms_yuv_buf_copy(uint8_t *src_planes[], const int src_strides[], uint8_t *dst_planes[], const int dst_strides[],
					 MSVideoSize roi) {
...
void ms_yuv_buf_copy_with_pix_strides(uint8_t *src_planes[], const int src_row_strides[], const int src_pix_strides[],
									  MSRect src_roi, uint8_t *dst_planes[], const int dst_row_strides[],
									  const int dst_pix_strides[], MSRect dst_roi) {
	plane_copy(src_planes[0], src_row_strides[0], src_pix_strides[0], &src_roi, dst_planes[0], dst_row_strides[0],
			   dst_pix_strides[0], &dst_roi);
	...
	plane_copy(src_planes[1], src_row_strides[1], src_pix_strides[1], &src_roi, dst_planes[1], dst_row_strides[1],
			   dst_pix_strides[1], &dst_roi);
	plane_copy(src_planes[2], src_row_strides[2], src_pix_strides[2], &src_roi, dst_planes[2], dst_row_strides[2],
			   dst_pix_strides[2], &dst_roi);
}
...
static MSVideoSize _ordered_vsizes[] = {{MS_VIDEO_SIZE_QCIF_W, MS_VIDEO_SIZE_QCIF_H},
...
static MSScalerContext *yuv_create_scale_context(int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h,
												 BCTBX_UNUSED(MSPixFmt dst_fmt), BCTBX_UNUSED(int flags)) {
...
static int yuv_scale(MSScalerContext *ctx, uint8_t *src[], int src_strides[], uint8_t *dst[], int dst_strides[]) {
	...
	switch (fctx->src_fmt) {
	case MS_YUV420P:
		err = I420Scale(src[0], src_strides[0], src[1], src_strides[1], src[2], src_strides[2], fctx->source.width,
						fctx->source.height, dst[0], dst_strides[0], dst[1], dst_strides[1], dst[2], dst_strides[2],
						fctx->target.width, fctx->target.height, kFilterBilinear);
		break;
	case MS_YUY2:
	case MS_YUYV:
		err = YUY2ToI420(src[0], src_strides[0], dst[0], dst_strides[0], dst[1], dst_strides[1], dst[2], dst_strides[2],
						 fctx->target.width, fctx->target.height);
		break;
		break;
	case MS_UYVY:
		err = UYVYToI420(src[0], src_strides[0], dst[0], dst_strides[0], dst[1], dst_strides[1], dst[2], dst_strides[2],
						 fctx->target.width, fctx->target.height);
		break;
...
MSScalerDesc yuv_scaler = {yuv_create_scale_context, yuv_scale, yuv_free};
#endif
...
static MSScalerContext *ff_create_swscale_context(int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h,
												  MSPixFmt dst_fmt, int flags) {
	int ff_flags = 0;
	...
	if (flags & MS_SCALER_METHOD_BILINEAR)
		...
		ff_flags |= SWS_BILINEAR;
#endif
	ctx->ctx = sws_getContext(src_w, src_h, ms_pix_fmt_to_ffmpeg(src_fmt), dst_w, dst_h, ms_pix_fmt_to_ffmpeg(dst_fmt),
							  ff_flags, NULL, NULL, NULL);
	if (ctx->ctx == NULL) {
		ms_free(ctx);
		ctx = NULL;
	}
...
static void ff_sws_free(MSScalerContext *ctx) {
	MSFFScalerContext *fctx = (MSFFScalerContext *)ctx;
	if (fctx->ctx)
		sws_freeContext(fctx->ctx);
	ms_free(ctx);
}

MSScalerDesc ffmpeg_scaler = {ff_create_swscale_context, ff_sws_scale, ff_sws_free};
...
MSScalerContext *ms_scaler_create_context(int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h,
										  MSPixFmt dst_fmt, int flags) {
...
static void rotate_plane_down_scale_by_2(int wDest, int hDest, int full_width, const uint8_t *src, uint8_t *dst,
										 int step, bool_t clockWise, bool_t downscale) {
...
mblk_t *copy_ycbcrbiplanar_to_true_yuv_with_rotation_and_down_scale_by_2(MSYuvBufAllocator *allocator, const uint8_t *y,
																		 const uint8_t *cbcr, int rotation, int w,
																		 int h, int y_byte_per_row,
																		 int cbcr_byte_per_row, bool_t uFirstvSecond,
																		 bool_t down_scale) {
	MSPicture pict;
	int uv_w = w / 2;
	int uv_h = h / 2;
...
#ifdef __ANDROID__
	if (hasNeon == -1) {
		hasNeon = (((android_getCpuFamily() == ANDROID_CPU_FAMILY_ARM) &&
					((android_getCpuFeatures() & ANDROID_CPU_ARM_FEATURE_NEON) != 0)) ||
				   (android_getCpuFamily() == ANDROID_CPU_FAMILY_ARM64));
#ifdef __aarch64__
		ms_warning(
			"Warning: ARM64 NEON routines for video rotation are not yet implemented for Android: using SOFT version!");
#endif
	}
#endif
	...
	if (rotation % 180 == 0) {
...
#if MS_HAS_ARM
			if (hasNeon) {
				deinterlace_down_scale_neon(y, cbcr, pict.planes[0], u_dest, v_dest, w, h, y_byte_per_row,
											cbcr_byte_per_row, down_scale);
			} else
#endif
			{
				// plain copy
...
#if defined(__arm__)
			if (hasNeon) {
				deinterlace_down_scale_and_rotate_180_neon(y, cbcr, pict.planes[0], u_dest, v_dest, w, h,
														   y_byte_per_row, cbcr_byte_per_row, down_scale);
			} else
#endif
			{
				// 180° y rotation
		...
		if (hasNeon) {
			if (clockwise) {
				...
				rotate_down_scale_plane_neon_anticlockwise(w, h, y_byte_per_row, (uint8_t *)y, pict.planes[0],
														   down_scale);
			}
		} else
#endif
...
#if defined(__arm__)
		if (hasNeon) {
			rotate_down_scale_cbcr_to_cr_cb(uv_w, uv_h, cbcr_byte_per_row / 2, (uint8_t *)cbcr, pict.planes[2],
											pict.planes[1], clockwise, down_scale);
		} else
#endif
		{
			// Copying U
...
mblk_t *copy_ycbcrbiplanar_to_true_yuv_with_rotation(MSYuvBufAllocator *allocator, const uint8_t *y,
													 const uint8_t *cbcr, int rotation, int w, int h,
													 int y_byte_per_row, int cbcr_byte_per_row, bool_t uFirstvSecond) {
	return copy_ycbcrbiplanar_to_true_yuv_with_rotation_and_down_scale_by_2(
		allocator, y, cbcr, rotation, w, h, y_byte_per_row, cbcr_byte_per_row, uFirstvSecond, FALSE);
}

mblk_t *copy_yuv_with_rotation(MSYuvBufAllocator *allocator, const uint8_t *y, const uint8_t *u, const uint8_t *v,
							   int rotation, int w, int h, int y_byte_per_row, int u_byte_per_row, int v_byte_per_row) {
	MSPicture pict;
	int uv_w = w / 2;
	int uv_h = h / 2;
	mblk_t *yuv_block;
	...
	if (rotation % 180 == 0) {
		...
		if (hasNeon) {
			if (clockwise) {
				...
				rotate_down_scale_plane_neon_anticlockwise(uv_w, uv_h, u_byte_per_row, (uint8_t *)u, pict.planes[1],
														   FALSE);
				rotate_down_scale_plane_neon_anticlockwise(uv_w, uv_h, v_byte_per_row, (uint8_t *)v, pict.planes[2],
														   FALSE);
			}
		} else
#endif
...
void ms_average_fps_init(MSAverageFPS *afps, const char *ctx) {
	...
	if (!ctx || strstr(ctx, "%f") == 0) {
		ms_error("Invalid MSAverageFPS context given '%s' (must be not null and must contain one occurence of '%%f'",
				 ctx);
	}
}
...
MSVideoConfiguration ms_video_find_best_configuration_for_bitrate(const MSVideoConfiguration *vconf_list, int bitrate,
	...
	while (TRUE) {
		...
		if ((cpu_count >= vconf_it->mincpu && bitrate >= vconf_it->required_bitrate) ||
			vconf_it->required_bitrate == 0) {
...
MSVideoConfiguration ms_video_find_best_configuration_for_size(const MSVideoConfiguration *vconf_list,
															   MSVideoSize vsize, int cpu_count) {
	const MSVideoConfiguration *vconf_it = vconf_list;
	MSVideoConfiguration best_vconf = {0};
...
MSVideoConfiguration ms_video_find_worst_configuration_for_size(const MSVideoConfiguration *vconf_list,
																MSVideoSize vsize, int cpu_count) {
	const MSVideoConfiguration *vconf_it = vconf_list;
	MSVideoConfiguration best_vconf = {0};
	int min_score = INT32_MAX;
	...
	/* search for configuration that is first nearest to target video size, then second has the lowest bitrate but the
	 * greater fps, but any case making sure the the cpu count is sufficient*/
	while (TRUE) {
		int pixels = vconf_it->vsize.width * vconf_it->vsize.height;
		int score = abs(pixels - ref_pixels);
		if (cpu_count >= vconf_it->mincpu) {
			if (score < min_score) {
				...
				if (best_vconf.required_bitrate == vconf_it->required_bitrate &&
					best_vconf.bitrate_limit == vconf_it->bitrate_limit) {
					if (best_vconf.fps < vconf_it->fps) {
						best_vconf = *vconf_it;
					}
				} else {
...
MSVideoConfiguration ms_video_find_best_configuration_for_size_and_bitrate(const MSVideoConfiguration *vconf_list,
																		   MSVideoSize vsize, int cpu_count,
																		   int bitrate) {
	const MSVideoConfiguration *vconf_it = vconf_list;
	MSVideoConfiguration best_vconf = {0};
	...
	if (bitrate == 0)
		return ms_video_find_best_configuration_for_size(vconf_list, vsize, cpu_count);

	/* Search for configuration that is first nearest to target video size, then target bitrate and finally has the
	 * greater fps, but any case making sure the cpu count is sufficient. We suppose that the configuration list is
	 * ordered. */
	while (TRUE) {
		int pixels = vconf_it->vsize.width * vconf_it->vsize.height;
		int score = abs(pixels - ref_pixels);
