...
int ms_yuv_buf_init_from_mblk(YuvBuf *buf, mblk_t *m) {
	...
	if (m->b_cont == NULL) ms_yuv_buf_init(buf, w, h, w, m->b_rptr);
	else ms_yuv_buf_init(buf, w, h, w, m->b_cont->b_rptr);
	return 0;
}
...
static void plane_copy(const uint8_t *src_plane,
                       size_t src_row_stride,
                       size_t src_pix_stride,
                       const MSRect *src_roi,
                       uint8_t *dst_plane,
                       size_t dst_row_stride,
                       size_t dst_pix_stride,
                       const MSRect *dst_roi) {
...
void ms_yuv_buf_copy_with_pix_strides(uint8_t *src_planes[],
                                      const int src_row_strides[],
                                      const int src_pix_strides[],
                                      MSRect src_roi,
                                      uint8_t *dst_planes[],
                                      const int dst_row_strides[],
                                      const int dst_pix_strides[],
                                      MSRect dst_roi) {
...
MSYuvBufAllocator *ms_yuv_buf_allocator_new(void) {
	...
	ms_yuv_buf_allocator_set_max_frames(allocator, 15); /* abitrary limit to avoid undefinite memory inflation */
	return allocator;
}
...
static MSScalerContext *yuv_create_scale_context(int src_w,
                                                 int src_h,
                                                 MSPixFmt src_fmt,
                                                 int dst_w,
                                                 int dst_h,
                                                 BCTBX_UNUSED(MSPixFmt dst_fmt),
...
static int yuv_scale(MSScalerContext *ctx, uint8_t *src[], int src_strides[], uint8_t *dst[], int dst_strides[]) {
	...
	switch (fctx->src_fmt) {
		...
		case MS_YUY2:
		case MS_YUYV:
			err = YUY2ToI420(src[0], src_strides[0], dst[0], dst_strides[0], dst[1], dst_strides[1], dst[2],
			                 dst_strides[2], fctx->target.width, fctx->target.height);
			break;
			break;
		case MS_UYVY:
			err = UYVYToI420(src[0], src_strides[0], dst[0], dst_strides[0], dst[1], dst_strides[1], dst[2],
			                 dst_strides[2], fctx->target.width, fctx->target.height);
			break;
...
static MSScalerContext *
ff_create_swscale_context(int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h, MSPixFmt dst_fmt, int flags) {
	int ff_flags = 0;
...
#pragma unused(flags)
	...
	else if (flags & MS_SCALER_METHOD_NEIGHBOUR) ff_flags |= SWS_BILINEAR;
#endif
...
MSScalerContext *
ms_scaler_create_context(int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h, MSPixFmt dst_fmt, int flags) {
	MSScalerDesc *impl = ms_video_get_scaler_impl();
	if (impl) return impl->create_context(src_w, src_h, src_fmt, dst_w, dst_h, dst_fmt, flags);
...
static void rotate_plane_down_scale_by_2(int wDest,
                                         int hDest,
                                         int full_width,
                                         const uint8_t *src,
                                         uint8_t *dst,
...
mblk_t *copy_ycbcrbiplanar_to_true_yuv_with_rotation(MSYuvBufAllocator *allocator,
                                                     const uint8_t *y,
                                                     const uint8_t *cbcr,
                                                     int rotation,
                                                     int w,
...
mblk_t *copy_yuv_with_rotation(MSYuvBufAllocator *allocator,
                               const uint8_t *y,
                               const uint8_t *u,
                               const uint8_t *v,
                               int rotation,
...
ms_video_find_best_configuration_for_bitrate(const MSVideoConfiguration *vconf_list, int bitrate, int cpu_count) {
...
MSVideoConfiguration
ms_video_find_best_configuration_for_size(const MSVideoConfiguration *vconf_list, MSVideoSize vsize, int cpu_count) {
	const MSVideoConfiguration *vconf_it = vconf_list;
	MSVideoConfiguration best_vconf = {0};
...
MSVideoConfiguration
ms_video_find_worst_configuration_for_size(const MSVideoConfiguration *vconf_list, MSVideoSize vsize, int cpu_count) {
	const MSVideoConfiguration *vconf_it = vconf_list;
	MSVideoConfiguration best_vconf = {0};
	int min_score = INT32_MAX;
	int ref_pixels = vsize.height * vsize.width;
