...
void ms_snd_card_manager_destroy(MSSndCardManager *scm) {
	if (scm != NULL && !bypass_sndcard_detection) {
		...
		for (elem = scm->descs; elem != NULL; elem = elem->next) {
			...
			if (desc->unload != NULL) desc->unload(scm);
		}
...
void ms_snd_card_manager_prepend_card(MSSndCardManager *m, MSSndCard *c) {
	ms_snd_card_set_manager(m, c);
	ms_message("Card '%s' prepended with capabilities [%s]", ms_snd_card_get_string_id(c),
	           cap_to_string(c->capabilities));
...
bool_t ms_snd_card_manager_swap_cards(MSSndCardManager *m, MSSndCard *card0, MSSndCard *card1) {
	...
	if (card0_found && card1_found) {
		...
		ms_message("[Card Swap] Unable to swap position of card '%s' and card '%s' because %s has not been found",
		           ms_snd_card_get_string_id(card0), ms_snd_card_get_string_id(card1),
		           (card0_found ? "latter" : "former"));
		return FALSE;
	}
}
...
bool_t ms_snd_card_manager_reload_requested(MSSndCardManager *m) {
	...
	for (elem = m->descs; elem != NULL; elem = elem->next) {
		MSSndCardDesc *desc = (MSSndCardDesc *)elem->data;
		if (desc->reload_requested != NULL && desc->reload_requested(m)) return TRUE;
	}
	return FALSE;
}
...
MSSndCard *ms_snd_card_dup(MSSndCard *card) {
	MSSndCard *obj = NULL;
	if (card->desc->duplicate != NULL) obj = card->desc->duplicate(card);
	return obj;
}
...
void ms_snd_card_set_level(MSSndCard *obj, MSSndCardMixerElem e, int percent) {
	if (obj->desc->set_level != NULL) obj->desc->set_level(obj, e, percent);
...
int ms_snd_card_get_level(MSSndCard *obj, MSSndCardMixerElem e) {
	if (obj->desc->get_level != NULL) return obj->desc->get_level(obj, e);
	else {
...
void ms_snd_card_set_capture(MSSndCard *obj, MSSndCardCapture c) {
	if (obj->desc->set_capture != NULL) obj->desc->set_capture(obj, c);
...
int ms_snd_card_set_control(MSSndCard *obj, MSSndCardControlElem e, int val) {
	if (obj->desc->set_control != NULL) return obj->desc->set_control(obj, e, val);
	else {
...
int ms_snd_card_get_control(MSSndCard *obj, MSSndCardControlElem e) {
	if (obj->desc->get_control != NULL) return obj->desc->get_control(obj, e);
	else {
...
struct _MSFilter *ms_snd_card_create_writer(MSSndCard *obj) {
	if (obj->desc->create_writer != NULL) return obj->desc->create_writer(obj);
...
void ms_snd_card_app_notifies_activation(MSSndCard *obj, bool_t yesno) {
	if (obj->desc->callkit_enabled != NULL) obj->desc->callkit_enabled(obj, yesno);
}

void ms_snd_card_configure_audio_session(MSSndCard *obj) {
	if (obj->desc->configure != NULL) obj->desc->configure(obj);
}
...
MSSndCard *ms_snd_card_get_card_duplicate(MSSndCardManager *m, MSSndCard *card, bool_t checkCapabilities) {
	...
	// Only one of them will be added to the card manager. In order to simplify the logic, the device added to the
	// manager will depend on the order devices are in the list. For any given combination of
	// <name>,<driver_type>,<device_type> only the first one inn the list will be added to the card manager If a card
...
void ms_snd_card_sort(MSSndCardManager *m) {
	...
	for (elem = m->cards; elem != NULL; elem = elem->next) {
		...
		if ((curr_driver == NULL) || (c->desc->driver_type && strcmp(c->desc->driver_type, curr_driver) != 0)) {
			...
			MSSndCard *earpiece =
			    ms_snd_card_manager_get_card_by_type(m, MS_SND_CARD_DEVICE_TYPE_EARPIECE, curr_driver);
		...
		// Check if c is not an earpiece or speaker - as these devices are built-in into the phone, there is only 1
		// instance for each of them and it is searched using ms_snd_card_manager_get_card_by_type
		if ((ms_snd_card_get_device_type(c) != MS_SND_CARD_DEVICE_TYPE_EARPIECE) &&
		    (ms_snd_card_get_device_type(c) != MS_SND_CARD_DEVICE_TYPE_SPEAKER)) {
			// Append card
			ltmp = bctbx_list_append(ltmp, c);
		}
	}
