...
void ms_filter_register(MSFilterDesc *desc) {
	...
	if (factory) {
		...
		ms_error("ms_filter_register(): registration of '%s' filter has failed: no fallback factory has been defined",
		         desc->name);
	}
}
...
bool_t ms_filter_implements_interface(MSFilter *f, MSFilterInterfaceId id) {
	if (f) return ms_filter_desc_implements_interface(f->desc, id);
	else return FALSE;
}
...
int ms_filter_link(MSFilter *f1, int pin1, MSFilter *f2, int pin2) {
	MSQueue *q;
	ms_message("ms_filter_link: %s:%p,%i-->%s:%p,%i", f1 ? f1->desc->name : "!NULL!", f1, pin1,
	           f2 ? f2->desc->name : "!NULL!", f2, pin2);
...
int ms_filter_unlink(MSFilter *f1, int pin1, MSFilter *f2, int pin2) {
	MSQueue *q;
	ms_message("ms_filter_unlink: %s:%p,%i-->%s:%p,%i", f1 ? f1->desc->name : "!NULL!", f1, pin1,
	           f2 ? f2->desc->name : "!NULL!", f2, pin2);
	ms_return_val_if_fail(pin1 < f1->desc->noutputs, -1);
	ms_return_val_if_fail(pin2 < f2->desc->ninputs, -1);
	ms_return_val_if_fail(f1->outputs[pin1] != NULL, -1);
...
int ms_filter_call_method(MSFilter *f, unsigned int id, void *arg) {
	/*compatibility stuff*/
	if (id == MS_AUDIO_DECODER_SET_RTP_PAYLOAD_PICKER &&
	    !ms_filter_has_method(f, MS_AUDIO_DECODER_SET_RTP_PAYLOAD_PICKER))
...
void ms_filter_destroy(MSFilter *f) {
	if (f->desc->uninit != NULL) f->desc->uninit(f);
...
void ms_filter_postprocess(MSFilter *f) {
	if (f->desc->postprocess != NULL) f->desc->postprocess(f);
	f->ticker = NULL;
}
