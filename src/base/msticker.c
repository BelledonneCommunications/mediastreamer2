...
static void ms_ticker_stop(MSTicker *s) {
	...
	if (s->thread) ms_thread_join(s->thread, NULL);
}
	...
	do {
		if (f->ticker == NULL) {
			...
			if (f->ticker == ticker) {
				...
				ms_fatal("MSTicker %p; cannot attach filter %s:%p : it is already being run by ticker %p.", ticker,
				         f->desc->name, f, f->ticker);
			}
		}
...
int ms_ticker_detach(MSTicker *ticker, MSFilter *f) {
	...
	if (f->ticker != ticker) {
		ms_fatal("ms_ticker_detach(): filter %s:%p is currently scheduled by MSTicker %p, but requested to detach from "
		         "MSTicker %p. This is a programming mistake.",
...
static void call_process(MSFilter *f) {
	...
	if (f->desc->ninputs == 0 || f->desc->flags & MS_FILTER_IS_PUMP) {
		...
		while (ms_filter_inputs_have_data(f)) {
			if (process_done) {
				ms_warning("Re-scheduling filter %s: all data should be consumed in one process call, so fix it.",
				           f->desc->name);
			}
			ms_filter_process(f);
...
static void run_graphs(MSTicker *s, bctbx_list_t *execution_list, bool_t force_schedule) {
	 ...
	 * that could not be scheduled (because they could not be scheduled themselves)... Do you understand ?*/
...
static int set_high_prio(MSTicker *obj) {
	...
	if (prio > MS_TICKER_PRIO_NORMAL) {
		...
		if (mm == 0) {
			if (ptc.wPeriodMin < (UINT)precision) ptc.wPeriodMin = precision;
			else precision = ptc.wPeriodMin;
		...
		if (prio == MS_TICKER_PRIO_REALTIME) policy = SCHED_FIFO;
...
#if TICKER_MEASUREMENTS
			ms_get_cur_time(&end);
			iload = 100 * ((end.tv_sec - begin.tv_sec) * 1000.0 + (end.tv_nsec - begin.tv_nsec) / 1000000.0) /
			        (double)s->interval;
...
double ms_ticker_synchronizer_update(MSTickerSynchronizer *ts, uint64_t nb_samples, unsigned int sample_rate) {
	/* It is important that the average clock skew is updated ONLY if the user notifies that nb_samples has changed.
	   Indeed, the fact that the soundcard didn't delivered samples during a period of time doesn't mean that the time
	   has stopped or slowed. What we need is that each time we get new samples, we correlate them with system time to
	   make a correction on the msticker timer. Imagine that the soundcard stops for some reason: without this check the
	   msticker would stop as well.
	*/
