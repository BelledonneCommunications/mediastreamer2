...
static void android_texture_display_destroy_opengl(AndroidTextureDisplay *ad) {
	ms_debug("[TextureView Display] Destroying context");
	if (ad->ogl) {
		ogl_display_uninit(ad->ogl, FALSE);
		ms_free(ad->ogl);
		ad->ogl = NULL;
		ms_debug("[TextureView Display] OGL context destroyed");
	}
	if (ad->gl_surface) {
		if (ad->gl_display) {
			...
			ad->gl_surface = NULL;
			ad->gl_display = NULL;
			ms_debug("[TextureView Display] OGL surface and display destroyed");
		}
	}
	if (ad->window) {
		ANativeWindow_release(ad->window);
		ad->window = NULL;
		ms_debug("[TextureView Display] Window released");
	}
}

static void android_texture_display_init_opengl(AndroidTextureDisplay *ad) {
   	JNIEnv *jenv = ms_get_jni_env();
	ms_debug("[TextureView Display] Initializing context");
...
static int android_texture_display_set_window(MSFilter *f, void *arg) {
	...
	ms_debug("[TextureView Display] set window jobject ptr is %p, previous one is %p", surface, ad->surface);
	ms_filter_lock(f);
	if (id == 0) {
		ad->surface = NULL;
		android_texture_display_destroy_opengl(ad);
