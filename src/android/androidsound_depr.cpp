...
void msandroid_sound_set_level(BCTBX_UNUSED(MSSndCard *card),
                               BCTBX_UNUSED(MSSndCardMixerElem e),
                               BCTBX_UNUSED(int percent)) {
}
...
static unsigned int get_supported_rate(unsigned int prefered_rate) {
	...
	int size = jni_env->CallStaticIntMethod(
	    audio_record_class, jni_env->GetStaticMethodID(audio_record_class, "getMinBufferSize", "(III)I"), prefered_rate,
...
static int set_read_rate(MSFilter *f, void *arg) {
	...
	if (d->rate == proposed_rate) return 0;
	else return -1;
}
...
MSFilterMethod msandroid_sound_read_methods[] = {{MS_FILTER_SET_SAMPLE_RATE, set_read_rate},
...
static void sound_read_setup(MSFilter *f) {
	...
	if (constructor_id == 0) {
		ms_error("cannot find AudioRecord (int audioSource, int sampleRateInHz, int channelConfig, int audioFormat, "
	...
	d->buff_size = jni_env->CallStaticIntMethod(d->audio_record_class, min_buff_size_id, d->rate,
	                                            2 /*CHANNEL_CONFIGURATION_MONO*/, 2 /*  ENCODING_PCM_16BIT */);
	d->read_chunk_size = d->buff_size / 4;
	d->buff_size *= 2; /*double the size for configuring the recorder: this does not affect latency but prevents
	                      "AudioRecordThread: buffer overflow"*/

	if (d->buff_size > 0) {
		ms_message("Configuring recorder with [%i] bits  rate [%i] nchanels [%i] buff size [%i], chunk size [%i]",
		           d->bits, d->rate, d->nchannels, d->buff_size, d->read_chunk_size);
	} else {
	...
	d->audio_record = jni_env->NewObject(d->audio_record_class, constructor_id,
	                                     sdk_version < 11 ? 1 /*MIC*/ : 7 /*VOICE_COMMUNICATION*/
	                                     ,
	                                     d->rate, 2 /*CHANNEL_CONFIGURATION_MONO*/
	                                     ,
	                                     2 /*  ENCODING_PCM_16BIT */
	                                     ,
	                                     d->buff_size);
...
static void sound_read_preprocess(MSFilter *f) {
	msandroid_sound_read_data *d = (msandroid_sound_read_data *)f->data;
	ms_debug("andsnd_read_preprocess");
	if (!d->started) sound_read_setup(f);
...
static void sound_read_process(MSFilter *f) {
	...
	do {
		if (can_output &&
		    (avail >=
		     nbytes *
		         2)) { // bytes*2 is to insure smooth output, we leave at least one packet in the buffer for next time*/
...
MSFilter *msandroid_sound_read_new(MSSndCard *card) {
	...
	if (card->device_description != NULL) {
		if (card->device_description->recommended_rate > 0) {
			data->rate = card->device_description->recommended_rate;
...
MSFilterMethod msandroid_sound_write_methods[] = {{MS_FILTER_SET_SAMPLE_RATE, set_write_rate},
                                                  {MS_FILTER_GET_SAMPLE_RATE, get_rate},
                                                  {MS_FILTER_SET_NCHANNELS, set_nchannels},
                                                  {MS_FILTER_GET_NCHANNELS, get_nchannels},
                                                  {0, NULL}};
...
public:
	msandroid_sound_write_data()
	    : audio_track_class(0), audio_track(0), write_chunk_size(0), writtenBytes(0), last_sample_date(0) {
...
void msandroid_sound_write_preprocess(MSFilter *f) {
	...
	d->buff_size = jni_env->CallStaticIntMethod(d->audio_track_class, min_buff_size_id, d->rate,
	                                            2 /*CHANNEL_CONFIGURATION_MONO*/, 2 /*  ENCODING_PCM_16BIT */);
	d->write_chunk_size = (d->rate * (d->bits / 8) * d->nchannels) * 0.02;
	// d->write_chunk_size=d->buff_size;
	if (d->buff_size > 0) {
		ms_message("Configuring player with [%i] bits  rate [%i] nchanels [%i] buff size [%i] chunk size [%i]", d->bits,
		           d->rate, d->nchannels, d->buff_size, d->write_chunk_size);
	} else {
		ms_message("Cannot configure player with [%i] bits  rate [%i] nchanels [%i] buff size [%i] chunk size [%i]",
		           d->bits, d->rate, d->nchannels, d->buff_size, d->write_chunk_size);
		return;
	}
	d->audio_track = jni_env->NewObject(d->audio_track_class, constructor_id, 0 /*STREAM_VOICE_CALL*/
	                                    ,
	                                    d->rate, 2 /*CHANNEL_CONFIGURATION_MONO*/
	                                    ,
	                                    2 /*  ENCODING_PCM_16BIT */
	                                    ,
	                                    d->buff_size, 1 /*MODE_STREAM */);
...
void msandroid_sound_write_process(MSFilter *f) {
	...
	while ((m = ms_queue_get(f->inputs[0])) != NULL) {
		if (d->started) {
			...
			if (d->sleeping) ms_cond_signal(&d->cond);
...
static int msandroid_hack_speaker_state(MSFilter *f, void *arg) {
	...
	jclass LinphoneCoreImpl_class =
	    (jclass)jni_env->NewGlobalRef(jni_env->FindClass("org/linphone/core/LinphoneCoreImpl"));
	if (LinphoneCoreImpl_class == 0) {
		ms_error("Cannot find org/linphone/core/LinphoneCoreImpl");
		return -1;
	}
	jmethodID getLc_id =
	    jni_env->GetStaticMethodID(LinphoneManager_class, "getLc", "()Lorg/linphone/core/LinphoneCore;");
	if (getLc_id == 0) {
		ms_error("Cannot find LinphoneManager.getLc()");
		return -1;
	}
	jmethodID routeAudioToSpeakerHelper_id =
	    jni_env->GetMethodID(LinphoneCoreImpl_class, "routeAudioToSpeakerHelper", "(Z)V");
