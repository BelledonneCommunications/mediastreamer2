 ...
 * If participants (MSAudioEndpoint) are using sampling rate different from the conference, then sample rate converters
 *are automatically added and configured. Participants can be removed from the conference with
 *ms_audio_conference_remove_member(). The conference processing is performed in a new thread run by a MSTicker object,
 *which is owned by the conference. When all participants are removed, the MSAudioConference object can then be safely
 *destroyed with ms_audio_conference_destroy().
 **/
 ...
 * sending and receiving audio through RTP, or a local participant, that is somebody using the local soundcard to
 ...
 * is_remote=TRUE.
 *
 * To create a local participant, first create and start an AudioStream with audio_stream_new() and
 *audio_stream_start_full(), with real soundcard arguments. Arguments controlling RTP should be filled with placeholders
 *value and will not be used for conferencing. Then, create a MSAudioEndpoint representing this local participant by
 *calling ms_audio_endpoint_get_from_stream() with the audiostream and is_remote=FALSE.<br> For example:<br> <PRE>
 ...
 * @note Endpoints created by ms_audio_endpoint_get_from_stream() must be released by
 ...
 * When all participants are removed, the MSVideoConference object can then be safely destroyed with
