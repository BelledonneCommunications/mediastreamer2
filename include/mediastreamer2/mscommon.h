 ...
 **/
MS2_PUBLIC MS2_DEPRECATED MSList *
ms_list_remove_custom(MSList *list, MSCompareFunc compare_func, const void *user_data);
...
MS2_PUBLIC MS2_DEPRECATED void
ms_list_for_each3(const MSList *list, MSIterate3Func iterate_func, void *user_data, void *factory);
 ...
 * \<libraryname\>_init. For example if a library 'libdummy.so' or 'libdummy.dll' is found, then the loader tries to
 * locate a C function called 'libdummy_init()' and calls it if it exists. ms_load_plugins() can be used to load
 * non-mediastreamer2 plugins as it does not expect mediastreamer2 specific entry points.
 *
 ...
 */
MS2_PUBLIC void ms_sound_device_description_add(const char *manufacturer,
                                                const char *model,
                                                const char *platform,
                                                unsigned int flags,
                                                int delay,
