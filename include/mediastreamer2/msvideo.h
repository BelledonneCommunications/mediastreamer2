...
struct _MSVideoConfiguration {
	...
	void *extra;          /**< A pointer to some extra parameters that may be used by the encoder when using this video
	                         configuration. */
};

#define MS_VIDEO_CONF(required_bitrate, bitrate_limit, resolution, fps, mincpu)                                        \
	{                                                                                                                  \
		required_bitrate, bitrate_limit, {MS_VIDEO_SIZE_##resolution##_W, MS_VIDEO_SIZE_##resolution##_H}, fps,        \
		    mincpu, NULL                                                                                               \
	}
...
#define MS_VIDEO_SIZE_UNKNOWN                                                                                          \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_UNKNOWN_W, MS_VIDEO_SIZE_UNKNOWN_H                                                               \
	}

#define MS_VIDEO_SIZE_CIF                                                                                              \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_CIF_W, MS_VIDEO_SIZE_CIF_H                                                                       \
	}
#define MS_VIDEO_SIZE_QCIF                                                                                             \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_QCIF_W, MS_VIDEO_SIZE_QCIF_H                                                                     \
	}
#define MS_VIDEO_SIZE_4CIF                                                                                             \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_4CIF_W, MS_VIDEO_SIZE_4CIF_H                                                                     \
	}
#define MS_VIDEO_SIZE_CVD                                                                                              \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_CVD_W, MS_VIDEO_SIZE_CVD_H                                                                       \
	}
#define MS_VIDEO_SIZE_QQVGA                                                                                            \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_QQVGA_W, MS_VIDEO_SIZE_QQVGA_H                                                                   \
	}
#define MS_VIDEO_SIZE_QVGA                                                                                             \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_QVGA_W, MS_VIDEO_SIZE_QVGA_H                                                                     \
	}
#define MS_VIDEO_SIZE_VGA                                                                                              \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_VGA_W, MS_VIDEO_SIZE_VGA_H                                                                       \
	}

#define MS_VIDEO_SIZE_IOS_MEDIUM                                                                                       \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_IOS_MEDIUM_W, MS_VIDEO_SIZE_IOS_MEDIUM_H                                                         \
	}

#define MS_VIDEO_SIZE_720P                                                                                             \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_720P_W, MS_VIDEO_SIZE_720P_H                                                                     \
	}

#define MS_VIDEO_SIZE_1080P                                                                                            \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_1080P_W, MS_VIDEO_SIZE_1080P_H                                                                   \
	}

#define MS_VIDEO_SIZE_NS1                                                                                              \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_NS1_W, MS_VIDEO_SIZE_NS1_H                                                                       \
	}

#define MS_VIDEO_SIZE_XGA                                                                                              \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_XGA_W, MS_VIDEO_SIZE_XGA_H                                                                       \
	}

#define MS_VIDEO_SIZE_SVGA                                                                                             \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_SVGA_W, MS_VIDEO_SIZE_SVGA_H                                                                     \
	}

#define MS_VIDEO_SIZE_SXGA_MINUS                                                                                       \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_SXGA_MINUS_W, MS_VIDEO_SIZE_SXGA_MINUS_H                                                         \
	}

#define MS_VIDEO_SIZE_UXGA                                                                                             \
	MS_VIDEO_SIZE_TYPE {                                                                                               \
		MS_VIDEO_SIZE_UXGA_W, MS_VIDEO_SIZE_UXGA_H                                                                     \
	}
...
#else
#define MS_VIDEO_SIZE_ASSIGN(vsize, name) vsize = MS_VIDEO_SIZE_##name
#endif
...
typedef enum MSVideoOrientation { MS_VIDEO_LANDSCAPE = 0, MS_VIDEO_PORTRAIT = 1 } MSVideoOrientation;
...
The returned mblk_t points to the external buffer, which is not copied, nor ref'd: the reference is simply transfered to
the returned mblk_t*/
MS2_PUBLIC mblk_t *ms_yuv_buf_alloc_from_buffer(int w, int h, mblk_t *buffer);
MS2_PUBLIC void ms_yuv_buf_copy(
    uint8_t *src_planes[], const int src_strides[], uint8_t *dst_planes[], const int dst_strides[], MSVideoSize roi);
MS2_PUBLIC void ms_yuv_buf_copy_with_pix_strides(uint8_t *src_planes[],
...
MS2_PUBLIC mblk_t *ms_yuv_allocator_get(MSYuvBufAllocator *obj, int size, int w, int h);
...
MS2_PUBLIC void deinterlace_and_rotate_180_neon(const uint8_t *ysrc,
                                                const uint8_t *cbcrsrc,
                                                uint8_t *ydst,
                                                uint8_t *udst,
                                                uint8_t *vdst,
                                                int w,
...
void deinterlace_down_scale_and_rotate_180_neon(const uint8_t *ysrc,
                                                const uint8_t *cbcrsrc,
                                                uint8_t *ydst,
                                                uint8_t *udst,
                                                uint8_t *vdst,
                                                ...
                                                int cbcr_byte_per_row,
                                                bool_t down_scale);
void deinterlace_down_scale_neon(const uint8_t *ysrc,
                                 const uint8_t *cbcrsrc,
                                 uint8_t *ydst,
                                 uint8_t *u_dst,
                                 uint8_t *v_dst,
...
/**
 * This functions copies a an YUV420 semi-planar (UV packed in a single plane) to normal YUV420 (three planes), by
 *eventually performing a rotation and a downscale by a factor of two.
 ...
 * @param uFirstvSecond a boolean to be set to TRUE if the UV plan starts with U, followed by V. This function can be
 *used to process YVU420 semi planar images.
 ...
 *
 * @REVISIT This function suffers from several deficiencies that makes it difficult to use. It could be reworked in the
 *following way:
 * - first rename it to have correct prefix ms_video_*something*, and use "yuv" wording rather than cbcr, to make it
 *consistent with other functions in mediastreamer.
 * - it should take as input the width and height of the source buffer. To be more concise, the MSPicture structure
 *could be use to represent the source image.
 * - it should not take the destination width and height, as they can be easily guessed from the source size and the
 *transformation requested.
 **/

MS2_PUBLIC mblk_t *copy_ycbcrbiplanar_to_true_yuv_with_rotation_and_down_scale_by_2(MSYuvBufAllocator *allocator,
                                                                                    const uint8_t *y,
                                                                                    const uint8_t *cbcr,
                                                                                    int rotation,
                                                                                    int dest_w,
...
struct _MSScalerDesc {
	MSScalerContext *(*create_context)(
	    int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h, MSPixFmt dst_fmt, int flags);
...
MS2_PUBLIC MSScalerContext *
ms_scaler_create_context(int src_w, int src_h, MSPixFmt src_fmt, int dst_w, int dst_h, MSPixFmt dst_fmt, int flags);

MS2_PUBLIC int
ms_scaler_process(MSScalerContext *ctx, uint8_t *src[], int src_strides[], uint8_t *dst[], int dst_strides[]);
 ...
 */
MS2_PUBLIC mblk_t *copy_ycbcrbiplanar_to_true_yuv_with_rotation(MSYuvBufAllocator *allocator,
                                                                const uint8_t *y,
                                                                const uint8_t *cbcr,
                                                                int rotation,
                                                                int w,
...
MS2_PUBLIC mblk_t *copy_yuv_with_rotation(MSYuvBufAllocator *allocator,
                                          const uint8_t *y,
                                          const uint8_t *u,
                                          const uint8_t *v,
                                          int rotation,
 ...
 */
MS2_PUBLIC MSVideoConfiguration ms_video_find_best_configuration_for_bitrate(const MSVideoConfiguration *vconf_list,
                                                                             int bitrate,
                                                                             int cpucount);
 ...
 * @return The best video configuration found in the given list.
 */
MS2_PUBLIC MSVideoConfiguration ms_video_find_best_configuration_for_size(const MSVideoConfiguration *vconf_list,
                                                                          MSVideoSize vsize,
                                                                          int cpucount);
 ...
 * @return The best video configuration found in the given list.
 */
MS2_PUBLIC MSVideoConfiguration ms_video_find_worst_configuration_for_size(const MSVideoConfiguration *vconf_list,
                                                                           MSVideoSize vsize,
                                                                           int cpucount);

MS2_PUBLIC MSVideoConfiguration ms_video_find_best_configuration_for_size_and_bitrate(
    const MSVideoConfiguration *vconf_list, MSVideoSize vsize, int cpu_count, int bitrate);
...
#define MS_FILTER_VIDEO_AUTO (-1)
#define MS_FILTER_VIDEO_NONE (0)
