...
static tone_test_def_t tone_definition[] = {{{"", 400, {2000, 0}, 0.6f, 0, 0}, {"", 2000, 300, 0.5f}},
...
void ms_tester_tone_generation_and_detection_loop(void) {
	...
	for (i = 0; i < (sizeof(tone_definition) / sizeof(tone_definition[0])); i++) {
		...
		BC_ASSERT_EQUAL(
		    0, ms_filter_call_method(ms_tester_tonedet, MS_TONE_DETECTOR_ADD_SCAN, &tone_definition[i].expected_tone),
		    int, "%d");
		BC_ASSERT_EQUAL(
		    0, ms_filter_call_method(ms_tester_dtmfgen, MS_DTMF_GEN_PLAY_CUSTOM, &tone_definition[i].generated_tone),
		    int, "%d");
		ms_sleep(1);
		BC_ASSERT_EQUAL(ms_tester_tone_detected, TRUE, int, "%d");
	}
...
bool_t wait_for_list_with_parse_events(
    bctbx_list_t *mss, int *counter, int value, int timeout_ms, bctbx_list_t *cbs, bctbx_list_t *ptrs) {
	bctbx_list_t *msi;
	bctbx_list_t *cbi;
	bctbx_list_t *ptri;
	int retry = 0;
