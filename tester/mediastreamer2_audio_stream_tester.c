...
static void participants_volumes_in_audio_stream(void) {
	...
	char* hello_file = bc_tester_res(HELLO_8K_1S_FILE);
	int dummy=0;
	MSFilterRequestAudioDataCb callback;
	...
	BC_ASSERT_EQUAL(audio_stream_get_participant_volume(margaux, 1), ms_volume_dbov_to_dbm0(-5), int, "%d");
	BC_ASSERT_EQUAL(audio_stream_get_participant_volume(margaux, 2), ms_volume_dbov_to_dbm0(-15), int, "%d");
	BC_ASSERT_EQUAL(audio_stream_get_participant_volume(margaux, 3), ms_volume_dbov_to_dbm0(-127), int, "%d");
