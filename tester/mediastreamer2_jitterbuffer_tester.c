...
void iterate_stats_logger(MediaStream *ms, BCTBX_UNUSED(void *user_pointer)) {
	ms_message("count=%u cum_loss=%ld late=%ld discarded=%lu", ms->sessions.rtp_session->rtp.jittctl.count,
...
void test_setup(int payload, OrtpJitterBufferAlgorithm algo) {
	...
	BC_ASSERT_EQUAL(
	    audio_stream_start_full(receiver, profile, 0, 0, 0, 0, payload, 50, NULL, RECORD_FILE, NULL, NULL, 0), 0, int,
	    "%d");
...
void test_clean(const char *file, int clock_rate) {
	...
	ms_message("file=%s, algo=%s, rate=%d, refresh_ms=%d, max_size_ms=%d\n"
	           ...
	           file, params.buffer_algorithm == OrtpJitterBufferRecursiveLeastSquare ? "rls" : "mean", clock_rate,
	           params.refresh_ms, params.max_size, (unsigned long)stats->bad, (unsigned long)stats->cum_packet_loss,
	           (unsigned long)stats->outoftime, (unsigned long)stats->discarded);
...
static void pcap_tester_streams_start(
    const PcapTesterContext *params, int audio_file_port, int audio_to_port, int video_file_port, int video_to_port) {
	...
	if (use_video) {
		...
		rtp_session_enable_congestion_detection(receiverv->ms.sessions.rtp_session,
		                                        params->enable_congestion_detection);

		BC_ASSERT_EQUAL(video_stream_start(receiverv, profile, 0, 0, 0, 0, params->video_payload, 50,
		                                   mediastreamer2_tester_get_mire(_factory)),
		                0, int, "%i");
	...
	if (use_audio) {
		running_streams++;
		if (ms_pcap_sendto(_factory, params->file, audio_file_port, &audio_dest, params->audio_clock_rate,
		                   params->ts_offset, end_of_pcap, NULL) == NULL) {
	...
	if (use_video) {
		running_streams++;
		if (ms_pcap_sendto(_factory, params->file, video_file_port, &video_dest, params->video_clock_rate,
		                   params->ts_offset, end_of_pcap, NULL) == NULL) {
...
void pcap_tester_audio_with_params(const PcapTesterContext *params) {
	pcap_tester_streams_start(params, 0, RECEIVER_RTP_PORT, -1, -1);
	pcap_tester_iterate_until();
	pcap_tester_stop(params);
}

void pcap_tester_audio(
    const char *file, OrtpJitterBufferAlgorithm algo, int congestion_count_expected, int clock_rate, int payload) {
...
void pcap_tester_video(
	...
	params.enable_congestion_detection = congestion_count_expected >= 0;
	pcap_tester_streams_start(&params, -1, -1, 0, RECEIVER_RTP_PORT);
	pcap_tester_iterate_until();
	pcap_tester_stop(&params);
}

static void ideal_network_basic(void) {
	pcap_tester_audio("./scenarios/pcmu_8k_no_jitter.pcap", OrtpJitterBufferBasic, -1, payload_type_pcmu8000.clock_rate,
	                  0);
...
static void chaotic_start_basic(void) {
	pcap_tester_audio("./scenarios/opus-poor-quality.pcapng", OrtpJitterBufferBasic, -1, payload_type_opus.clock_rate,
	                  OPUS_PAYLOAD_TYPE);
...
static void congestion_detector_ideal_video(void) {
	...
	pcap_tester_streams_start(&params, 7078, RECEIVER_RTP_PORT, 9078, RECEIVER_RTP_PORT + 1234);
	pcap_tester_iterate_until();
	pcap_tester_stop(&params);
}
static void congestion_detector_limited_video_800kbits(void) {
	...
	pcap_tester_streams_start(&params, -1, 0, 9078, RECEIVER_RTP_PORT + 1234);
	pcap_tester_iterate_until();
	pcap_tester_stop(&params);
}
...
void congestion_adaptation(OrtpJitterBufferAlgorithm algo) {
	...
	BC_ASSERT_EQUAL(audio_stream_start_full(sender, profile, RECEIVER_IP, RECEIVER_RTP_PORT, RECEIVER_IP, 0, 0, 50,
	                                        hello_file, NULL, NULL, NULL, 0),
	                0, int, "%i");
	...
	// wait_for_until_with_parse_events(&receiver->ms,&sender->ms,
	// &netsim.enabled,1e7,30000,iterate_stats_logger,&sender->ms.sessions.rtp_session->cum_loss,NULL,NULL);
	...
	// wait_for_until_with_parse_events(&receiver->ms,NULL, &netsim.enabled, 180,
	// 2000,iterate_stats_logger,NULL,NULL,NULL);
	init_time = receiver->ms.sessions.ticker->get_cur_time_ptr(receiver->ms.sessions.ticker);
